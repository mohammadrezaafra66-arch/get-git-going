...
3. `zz-10-afrakala-roles.sh` — بعد از migration رسمی، وجود roleهای مورد انتظار
   ...
   login را normalize می‌کند. این اسکریپت با `psql -U supabase_admin` از طریق
   Unix socket مسیر `/var/run/postgresql` با `PGPASSWORD=POSTGRES_PASSWORD` وصل می‌شود، چون extension
