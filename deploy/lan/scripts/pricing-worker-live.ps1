# pricing-worker-live.ps1
# Drains pricing_recompute_queue via the public process-pricing-queue hook.
# Reads PRICING_WORKER_TOKEN from deploy/lan/.env.lan — never prints it.
...
$logFile = Join-Path $logDir "pricing-worker-live.log"
...
if (-not $token) {
  throw "PRICING_WORKER_TOKEN missing in .env.lan"
}

$appPort = Get-EnvValue $envFile "APP_PORT"
if (-not $appPort) { $appPort = "3100" }
$endpoint = "http://127.0.0.1:{0}/api/public/hooks/process-pricing-queue" -f $appPort
$body = '{"batch_size":50,"max_attempts":3}'
