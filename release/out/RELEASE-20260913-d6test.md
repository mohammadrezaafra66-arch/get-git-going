...
before the first Expect: in the document so nothing else can run first.
    ...
    $buildSha = '<sha: supply -BuildManifest>'
...
Expect: OK D1a, HEAD equal to the build sha <sha: supply -BuildManifest>, working tree clean

    # D1(b) -- every migration file in THIS release set must exist on disk.
    $expected = @('20260828000000_411_customer_credit_ranges_widened_and_scores_recomputed.sql', '20260828010000_412_cooperation_hint_matches_the_widened_range.sql', '20260829000000_413_salesperson_scoring_ranges_widened_and_scores_recomputed.sql', '20260913091000_527_drop_dead_receipt_posting_path_no_guard.sql', '20260913092000_528_posted_entry_immutability_no_guard.sql', '20260913094000_530_overdue_sensor_covers_unknown_due_date.sql', '20260913095000_531_audit_logs_actor_fk_set_null_on_delete.sql', '20260913100000_532_drop_duplicate_signup_trigger.sql', '20260913101000_533_pg_cron_http_scheduler.sql', '20260913102000_534_cron_run_log.sql', '20260913103000_535_security3_s5_function_fixes.sql', '20260913104000_536_ai_providers_updated_by.sql', '20260913105000_537_revoke_truncate_from_authenticated.sql', '20260913110000_538_close_anon_execute_on_pre393_functions.sql', '20260913111000_539_explicit_revoke_on_526_535_definers.sql')
    $missing = @()
...
Expect: OK D1b, all 15 migration files present, zero missing
...
### Block 24 - ledger-row-only BY DECISION (decision file not supplied) - 20260822210000 . 20260822210000_373_close_anon_default_privileges.sql
...
### Block 25 - SKIPPED BY DECISION (decision file not supplied) - DO NOT RUN - 20260905171000 . 20260905171000_449_retire_daily_capital_functions.sql
...
Expect: 0 (no ledger row, permanently, by decision (decision file not supplied))
...
### Block 26 - SKIPPED BY DECISION (decision file not supplied) - DO NOT RUN - 20260905171500 . 20260905171500_450_retire_superseded_tables.sql
...
### Block 27 - SKIPPED BY DECISION (decision file not supplied) - DO NOT RUN - 20260905180000 . 20260905180000_452_retire_parameter_weight_backups_by_rename.sql
...
host" -- it is "does it contain ANY host", which is a stronger and simpler property.
    ...
    $img    = 'afrakala-app:<sha: supply -BuildManifest>'
...
Expect: OK D6(iii), served config naming 192.168.170.10

# Phase 6 - deploy

### Block 30 - rollback tag
...
### Block 33 - rollback (run this instead of Block 31 if Block 32 fails)
...
Overall: PASSED / STOP  (circle one; STOP means Block 33's rollback was used)
