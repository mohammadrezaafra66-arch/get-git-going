-- Phase 5: sales search shows a price per settlement term.
-- Return signature is UNCHANGED (prices jsonb) — only the content of each
-- product's `prices` array is enriched. Each element gains
-- settlement_type_id + settlement_title. The array is built as:
--   * baseline rows  (settlement_type_id NULL) — exactly today's behavior,
--     one per active sale_price_type (kept even when price is NULL so the UI
--     still shows "قیمت ثبت نشده");
--   * per-settlement rows — one per (active sale type × active settlement)
--     that actually has a computed price, so sale types without a
--     settlement-specific rule don't flood the card with empty rows.
