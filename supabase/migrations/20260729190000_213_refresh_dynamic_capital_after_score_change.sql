...
-- 213 - Keep customer credit usable after scoring values change.
--
-- Problem found on LAN:
-- daily_capital_settings for 2026-07-29 was created before salesperson scores
-- were entered. calculate_dynamic_score() returned non-zero salesperson scores
-- afterwards, but salesperson_capital_allocations_dynamic still held the old
-- zero snapshot, so calculate_customer_realtime_credit() returned final_limit=0.
--
-- Safety:
-- * No existing migration is edited.
-- * No table rows are deleted.
-- * Existing allocation rows are updated in place; missing rows are inserted.
-- * If capital_allocation_ledger has any row for the target snapshot, the
--   recompute is skipped and audited instead of overwriting live held/consumed
--   business state.
--
-- Rollback:
--   DROP TRIGGER IF EXISTS trg_refresh_dyn_capital_after_score_change ON public.dynamic_entity_scores;
--   DROP FUNCTION IF EXISTS public.refresh_today_dynamic_capital_after_score_change();
--   DROP FUNCTION IF EXISTS public.recompute_dynamic_capital_setting(uuid, text);
...
BEGIN
  ...
  IF v_sum_sp_score > 0 THEN
    ...
    UPDATE _sp_alloc SET allocated_capital = floor_amount;
  ...
  LOOP
    ...
    IF v_sum_cust_score > 0 THEN
      ...
      UPDATE _sp_cust SET raw_allocation = floor_amount;
...
CREATE OR REPLACE FUNCTION public.refresh_today_dynamic_capital_after_score_change()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
DECLARE
  ...
  v_setting_id uuid;
BEGIN
  IF v_score_period <> v_period THEN
  ...
  PERFORM public.recompute_dynamic_capital_setting(
    v_setting_id,
    'dynamic_entity_score_' || lower(TG_OP)
  );
...
DROP TRIGGER IF EXISTS trg_refresh_dyn_capital_after_score_change
  ON public.dynamic_entity_scores;

CREATE TRIGGER trg_refresh_dyn_capital_after_score_change
  AFTER INSERT OR UPDATE OR DELETE ON public.dynamic_entity_scores
  FOR EACH ROW
  EXECUTE FUNCTION public.refresh_today_dynamic_capital_after_score_change();

COMMENT ON FUNCTION public.recompute_dynamic_capital_setting(uuid, text)
IS 'Item 213: safely recomputes an existing dynamic capital snapshot after scoring values change, without deleting business rows.';
