...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'marketing_channels_daily_quota_nonneg'
  ) THEN
    ALTER TABLE public.marketing_channels
      ADD CONSTRAINT marketing_channels_daily_quota_nonneg
      CHECK (daily_quota IS NULL OR daily_quota >= 0);
  END IF;
END $$;

-- Helper index for daily usage lookups
CREATE INDEX IF NOT EXISTS idx_audit_promo_used_day
  ON public.audit_logs (action, created_at)
  WHERE action = 'promotion_suggestion_used';

-- Rebuild view to include daily quota usage
