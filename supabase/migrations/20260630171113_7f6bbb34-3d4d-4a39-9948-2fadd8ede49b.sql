...
BEGIN
  ...
  IF v_sum_sp_score > 0 THEN
    ...
    IF v_remainder > 0 THEN
      ...
      UPDATE _sp_alloc s
      SET allocated_capital = s.floor_amount + 1
      FROM ranked r
      WHERE s.salesperson_id = r.salesperson_id;
    END IF;
  END IF;
  ...
  LOOP
    ...
    INSERT INTO _cust_alloc(
      customer_id, salesperson_id, weighted_score, share_ratio,
      raw_allocation, credit_limit, has_overdue, has_profile
    )
    SELECT
      sc.customer_id,
      v_sp.salesperson_id,
      ...
      sc.raw_allocation,
      ccp.credit_limit,
      COALESCE(ccp.has_overdue, false),
      ccp.id IS NOT NULL
    FROM _sp_cust sc
    LEFT JOIN public.customer_credit_profile ccp ON ccp.customer_id = sc.customer_id;
  ...
  UPDATE _cust_alloc SET
    final_limit = CASE
      ...
      WHEN NOT has_profile THEN 0
      WHEN credit_limit IS NULL THEN raw_allocation
      WHEN raw_allocation <= credit_limit THEN raw_allocation
      ELSE credit_limit
    END,
    binding_constraint = CASE
      WHEN has_overdue THEN 'overdue'
      WHEN NOT has_profile THEN 'credit_limit'
      WHEN credit_limit IS NULL THEN 'formula'
      WHEN raw_allocation <= credit_limit THEN 'formula'
      ELSE 'credit_limit'
    END;

  INSERT INTO public.customer_capital_allocations_dynamic(
    capital_setting_id, customer_id, salesperson_id,
    weighted_score, share_ratio, raw_allocation, final_limit, binding_constraint
  )
  SELECT
    v_setting_id, customer_id, salesperson_id,
    weighted_score, share_ratio, raw_allocation, final_limit, binding_constraint
  FROM _cust_alloc;
...
GRANT EXECUTE ON FUNCTION public.run_daily_capital_allocation(date, numeric, text) TO authenticated, service_role;
