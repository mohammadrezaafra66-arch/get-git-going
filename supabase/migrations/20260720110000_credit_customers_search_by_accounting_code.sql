-- =====================================================================
-- Migration: credit_customers_search_by_accounting_code  (Phase F)
--
-- Goal: on the «مشتریان معتبر» (credit) page, let the search box also match a
--   customer's «کد آسان» (customers.accounting_code), in addition to name.
--
-- Two changes to list_trusted_credit_customers:
--   1. Populate `easy_code` from `c.accounting_code` (was hardcoded NULL, so
--      the column never displayed and could not be searched).
--   2. Extend the `p_search` filter to also match `easy_code` via ILIKE
--      (partial match, which also covers exact match).
--
-- Everything else (authz, filters, ordering, grants) is unchanged.
-- Idempotent: CREATE OR REPLACE. Signature unchanged → no types.ts change.
-- After applying: docker restart afrakala-lan-rest. Connect as supabase_admin.
-- =====================================================================
