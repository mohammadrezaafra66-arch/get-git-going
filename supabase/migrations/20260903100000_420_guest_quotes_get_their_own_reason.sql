...
-- 420 — a quote with no customer file gets its own reason, instead of borrowing
--       'accounting_approval' from a department that never approved it.
--
-- WHY. Until now the ONLY way to record a quote with no customer_id was to send
-- 'accounting_approval', because the function rejects every other value on that branch.
-- So documents nobody in accounting ever saw are stored as accounting approvals.
-- Measured before this migration: of 18 accounting_approval quotes, only 2 are real
-- guests (credit_check_snapshot->>'mode' = 'guest_accounting_approval').
--
-- WHAT THIS IS NOT. It is not a way around the mandatory phone. A walk-in HAS a phone;
-- what they lack is a file in the register. sales_quotes.customer_phone stays NOT NULL
-- and src/lib/sales/quotes.ts:98 keeps rejecting a blank one.
--
-- SCOPE. Three allowed-set widenings and one comment. The function's signature, owner,
-- SECURITY DEFINER, search_path and GRANTs are untouched — the body below was generated
-- by applying exactly two replacements to pg_get_functiondef() output captured before
-- the change; see docs/release/420-function-diff.txt for the complete diff.
--
-- DELIBERATELY NOT WIDENED: the branch for a LINKED customer with no credit allocation
-- (the ELSIF at the same nesting depth) still demands 'accounting_approval'. Letting a
-- linked customer take the guest door would be a credit-control bypass, not a fix.
--
-- ROLLBACK: docs/release/420-rollback-create_sales_quote_with_items.sql restores the
-- function; the constraint reverts by re-running the ADD below with the 3-value list.

-- No explicit BEGIN/COMMIT: this file is applied with psql --single-transaction, which
-- supplies the wrapper. An inner COMMIT would end that transaction early and quietly
-- forfeit the rollback guarantee the runbook depends on.

-- 1. The table CHECK. Widening a CHECK has no ALTER form in PostgreSQL, so the drop and
--    re-add is the only idiom; it is a constraint, not a column or a table, and both
--    statements are inside this transaction. Every existing row satisfies the wider set,
--    so the re-add validates without touching data.
ALTER TABLE public.sales_quotes DROP CONSTRAINT sales_quotes_exception_type_check;
ALTER TABLE public.sales_quotes ADD CONSTRAINT sales_quotes_exception_type_check
  CHECK (
    quote_exception_type IS NULL
    OR quote_exception_type = ANY (ARRAY[
      'overdue_salesperson_commitment'::text,
      'credit_shortfall_salesperson_commitment'::text,
      'accounting_approval'::text,
      'guest_no_link'::text
    ])
  );

-- 2. The function's own two allowed sets.
...
BEGIN
  ...
  FOR _item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    END IF;
    -- 415 — a quote line must name a product that exists. 'manual' and 'quick_price'
    -- both accepted a typed-in name with no product_id, which is the same hole under two
    -- labels; the owner's rule is that only accounting creates products.
    IF _src IN ('manual','quick_price') THEN
      RAISE EXCEPTION 'این کالا در سیستم تعریف نشده است. برای ثبت پیش‌فاکتور، ابتدا محصول باید توسط حسابداری ساخته شود.'
        USING ERRCODE = '22023';
    END IF;
  ...
  IF p_customer_id IS NULL THEN
    IF p_quote_exception_type IS DISTINCT FROM 'accounting_approval'
       AND p_quote_exception_type IS DISTINCT FROM 'guest_no_link' THEN
...
-- 3. Metadata only, and the reason it is here: commitment_confirmed reads like a general
--    attestation and is not one. Its display on the quote detail page is nested inside the
--    deposit block (renders only when deposit_amount > 0) and its intended writer is the
--    30%-deposit checkbox in AdvancePaymentSection.tsx, a component with zero references.
--    It is false on all 63 quotes. Mistaking it for a general commitment flag cost a full
--    discovery cycle on 2026-09-03; this comment is so it does not cost the next person one.
COMMENT ON COLUMN public.sales_quotes.commitment_confirmed IS
  'تعهد مربوط به بیعانه (حداقل ۳۰٪) — تعهد عمومی نیست. نویسندهٔ موردنظرش AdvancePaymentSection است که هنوز وصل نشده، و نمایشش در صفحهٔ جزئیات فقط داخل بلوک بیعانه است. برای پذیرش تعهد ثبت مهمان از ردیف audit_logs با action=sales_quote_guest_no_link استفاده کنید، نه از این ستون.';
