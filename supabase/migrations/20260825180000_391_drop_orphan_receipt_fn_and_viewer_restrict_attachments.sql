...
-- ============================================================================= ITEM 1
DROP FUNCTION public.trg_post_receipt_on_approve();
...
BEGIN
  ...
  IF to_regclass('public.document_attachments') IS NULL THEN
    RAISE EXCEPTION '391 FAILED A0: public.document_attachments does not exist, so every policy check below would pass vacuously.';
  END IF;

  IF NOT (SELECT c.relrowsecurity FROM pg_class c WHERE c.oid = 'public.document_attachments'::regclass) THEN
    RAISE EXCEPTION '391 FAILED A0: RLS is DISABLED on public.document_attachments. A restrictive policy on a table with RLS off is decorative -- it is never consulted.';
  END IF;

  IF to_regproc('public.is_viewer_only') IS NULL THEN
    RAISE EXCEPTION '391 FAILED A0: is_viewer_only does not exist; the policy predicate would not be evaluable.';
  END IF;
  ...
  IF NOT public.is_viewer_only(v_viewer) THEN
    RAISE EXCEPTION '391 FAILED A0: probe user % is no longer viewer-only, so the closed-half of this gate cannot speak. Re-pick a viewer-only account.', v_viewer;
  END IF;
  IF public.is_viewer_only(v_admin) OR public.is_viewer_only(v_acct) OR public.is_viewer_only(v_manager) THEN
    RAISE EXCEPTION '391 FAILED A0: one of the privileged probe users is now viewer-only; the open-half of this gate would fail for the wrong reason.';
  END IF;

  SELECT id INTO v_receipt FROM public.payment_receipts ORDER BY created_at LIMIT 1;
  IF v_receipt IS NULL THEN
    RAISE EXCEPTION '391 FAILED A0: payment_receipts is empty, so no valid document_id exists and the probe INSERT (which trigger validate_document_attachment_ref checks by FK-in-a-trigger) cannot run.';
  END IF;
  ...
  IF v_seen IS NULL OR v_seen <> 0 THEN
    RAISE EXCEPTION '391 FAILED D: a viewer-only account reads % row(s) from document_attachments. These are financial attachments -- receipt images carrying amounts and account numbers.', coalesce(v_seen, -1);
  END IF;
  ...
  IF coalesce(v_admin_n,0) = 0 OR coalesce(v_acct_n,0) = 0 OR coalesce(v_mgr_n,0) = 0 THEN
    RAISE EXCEPTION '391 FAILED D: the probe row is invisible to admin (%), accountant (%) or manager (%). This migration must close the viewer and change nothing else; emptying the table for every role is a regression, not a fix.',
  ...
  IF coalesce(v_sales_n, -1) <> 0 THEN
    RAISE EXCEPTION '391 FAILED D: a sales account now reads % row(s); it read 0 before this migration. This file must not widen anything.', v_sales_n;
  END IF;
