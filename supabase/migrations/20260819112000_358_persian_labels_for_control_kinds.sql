-- 358 — no English account_kind reaches the accountant (Gate A phase 3, M2)
--
-- ==============================================================================================
-- THE DEFECT
-- ==============================================================================================
--
-- A cheque payment is blocked from the Asan export with a raw English identifier inside a Persian
-- sentence. Reproduced by phase-3 Gate A:
--
--   asan_list_journal_export(current_date, current_date, 'all') for a cheque payment
--     -> doc_kind = unclassified
--     -> blocked_reason = کد حساب آسان برای «cheque_payable» ثبت نشده است
--
-- D16 makes Persian messages part of the contract. This is phase-2 Gate A defect m3 — raised then
-- for cheque_receivable and left open — reproduced on a surface phase 3 created.
--
-- ==============================================================================================
-- WHY THE OBVIOUS FIX IS NOT AVAILABLE — recorded, because it was tried first
-- ==============================================================================================
--
-- Phase-2 m3's recommendation was "seed asan_control_accounts with a label_fa for every
-- account_kind in the CHECK, even where accounting_code stays NULL". That cannot be done. Measured
-- from the live catalogue after the attempt failed:
--
...
--
-- So the table cannot hold a row for cheque_receivable or cheque_payable at all, and no row of any
-- kind may carry a NULL code. Following m3's recommendation literally would have required widening
-- that CHECK and dropping a NOT NULL on a configuration table whose entire purpose is to hold Asan
-- codes — a much larger change than the message defect warrants, and one that would let a
-- code-less row masquerade as configured.
--
-- The label therefore has to come from the expression that builds it. That is what this migration
-- changes, and nothing else.
--
-- ==============================================================================================
-- MESSAGE ONLY — the boundary this migration does not cross
-- ==============================================================================================
--
-- asan_list_journal_export resolves two things per line: acode (the Asan code) and aname (the
-- display name). Blocking is decided by acode:
--
--   bad_kind / bad_name are selected  FILTER (WHERE l.acode IS NULL)
--
-- This migration touches ONLY the aname fallback. acode is untouched, the CASE that classifies
-- doc_kind is untouched, and the bank-sign heuristic is untouched. The same cheque document is
-- still blocked and still classifies as 'unclassified'; only the NAME inside the message changes
-- from an English identifier to Persian. What the export blocks and how it classifies is phase 5's
-- surface and D8's decision, and neither moves here. Verified after applying, below.
--
-- The configured label still wins: asan_control_accounts.label_fa is consulted first, so anything
-- an operator enters overrides these built-in names. And the raw identifier remains the LAST
-- resort, on purpose — if a future account_kind is added to the CHECK and nobody names it, the
-- identifier surfacing is what makes the omission visible, where a silent «؟» would hide it.
--
-- ==============================================================================================
-- WHAT WRITES OR DEPENDS ON asan_list_journal_export  (README-EXECUTION §H, first half)
-- ==============================================================================================
--
--   Callers: src/lib/asan/export-journal.ts (the /admin/asan-export page, three of its six options
--            come from this one function differing only by filter — ground-truth §7). No SQL
--            function calls it. It is read-only: it writes nothing.
--   Signature unchanged — (date, date, text) RETURNS TABLE(...16 columns) — so no caller changes
--   and no overload is created (CLAUDE.md rule 5).
--   Body generated FROM THE LIVE DEFINITION with exactly the one expression replaced (rule 6: the
--   live definition is the truth, the repo file may be stale), so what is deployed and what is
--   reviewed are the same text.
--
-- WHAT WILL READ THE ROWS THIS AFFECTS (§H, second half): the export produces no rows of its own.
-- Its output is read by the export page and, after that, by an accountant. The change makes one
-- string in that output Persian.
--
-- Rollback: docs/verification/358-down.sql — statements only; restores the previous fallback.
