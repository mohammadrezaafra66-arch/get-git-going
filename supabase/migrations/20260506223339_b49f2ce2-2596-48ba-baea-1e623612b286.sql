-- Phase 21.6C: blend collected sales (80%) with issued sales (20%) into employee score (monthly only)
...
BEGIN
  ...
  -- Phase 21.6C: collected sales (6-month window) for blended monthly sales
  -- inline computation to avoid RBAC interaction with calculate_salesperson_collected_sales
  ...
  -- 80% collected (6m) + 20% issued (monthly). When user is not a salesperson both are 0 → no harm.
  ...
  FOR _kpi IN SELECT key, weight FROM public.gamification_kpis WHERE enabled=true LOOP
    ...
    CASE _kpi.key
      ...
      -- Phase 21.6C: total_sales (monthly) is the 80/20 blend; daily/weekly/total stay issued-based for back-compat
  ...
  -- Phase 21.6C breakdown extension
...
'Phase 21.6C: monthly total_sales KPI is now a blend of 80% collected sales (6-month window, capped per invoice, status approved/verified/confirmed/posted) and 20% issued invoices. Daily/weekly/total remain issued-based for back-compat. Non-salespeople yield 0 from both sides — safe.';