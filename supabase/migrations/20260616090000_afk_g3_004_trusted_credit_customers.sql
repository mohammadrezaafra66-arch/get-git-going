-- AFK-G3-004: trusted credit customers read model
-- Scope: sanitized, read-only RPC for internal users to see trusted customers
-- and the computed account-sale limit without exposing sensitive credit internals.
...
AS $function$
  ...
  filtered AS (
    SELECT *
    FROM statused s
    WHERE (COALESCE(NULLIF(BTRIM(p_search), ''), '') = '' OR s.customer_name ILIKE '%' || BTRIM(p_search) || '%')
...
'AFK-G3-004: Sanitized read model for trusted credit customers. Exposes safe computed account-sale limit to internal authenticated users without granting direct customer_credit_profile sensitive access.';
