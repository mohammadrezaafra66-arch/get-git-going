...
-- ============================================================================
-- 577 - create_sales_quote_with_items: optional p_interaction_id (C9 / F3)
-- ============================================================================
-- After create, FE used to UPDATE sales_quotes SET interaction_id + salesperson_id.
-- authenticated has no UPDATE on sales_quotes (mig SF-1.c4 / 20260617052612), so
-- sales JWT got: permission denied for table sales_quotes.
--
-- Approach A: extend SECURITY DEFINER RPC with p_interaction_id (default NULL).
-- When set: validate deal exists + has salesperson_id; INSERT with
-- interaction_id + salesperson_id = deal.salesperson_id. Status stays DEFAULT draft.
-- When NULL: unchanged (salesperson_id = auth.uid(), interaction_id NULL).
--
-- Live pre-change: docs/missions/salesdesk-9-fixes/evidence/FIX/create_sales_quote_with_items-live-pre577.txt
-- (identical to mig 421 body aside from trailing semicolon).
-- Rollback: docs/missions/salesdesk-9-fixes/revert/577_create_sales_quote_link_deal.sql
-- ============================================================================
...
CREATE OR REPLACE FUNCTION public.create_sales_quote_with_items(p_customer_name text, p_customer_phone text, p_customer_note text, p_expires_at timestamp with time zone, p_subtotal_amount numeric, p_discount_amount numeric, p_final_amount numeric, p_items jsonb, p_settlement_type_id uuid DEFAULT NULL::uuid, p_customer_id uuid DEFAULT NULL::uuid, p_below_list_ack boolean DEFAULT false, p_deposit_amount numeric DEFAULT NULL::numeric, p_commitment_confirmed boolean DEFAULT false, p_visitor_id uuid DEFAULT NULL::uuid, p_warehouse_id uuid DEFAULT NULL::uuid, p_quote_exception_type text DEFAULT NULL::text, p_quote_exception_minutes integer DEFAULT NULL::integer, p_quote_exception_amount numeric DEFAULT NULL::numeric, p_quote_exception_text text DEFAULT NULL::text, p_interaction_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  _wh uuid;
  _interaction_id uuid := NULL;
  _quote_salesperson_id uuid;
  _deal_id uuid;
  _deal_salesperson_id uuid;
  _need record;
  _available numeric;
BEGIN
  ...
  IF p_customer_id IS NULL THEN
    IF p_quote_exception_type IS DISTINCT FROM 'accounting_approval'
      ...
      RAISE EXCEPTION 'این پیش‌فاکتور به پرونده مشتری ثبت‌شده وصل نیست. برای ثبت، یا مشتری را به پرونده وصل کنید، یا مسیر مهمان را با پذیرش تعهد انتخاب کنید، یا تأیید حسابداری بگیرید.'
        USING ERRCODE = '22023';
    END IF;
  ...
  -- 577 / C9: optional deal link — salesperson from deal row, not client UPDATE
  _quote_salesperson_id := _uid;
  IF p_interaction_id IS NOT NULL THEN
    SELECT si.id, si.salesperson_id
      INTO _deal_id, _deal_salesperson_id
      FROM public.sales_interactions si
     WHERE si.id = p_interaction_id;

    IF _deal_id IS NULL THEN
      RAISE EXCEPTION 'معامله پیدا نشد.' USING ERRCODE = 'P0002';
    END IF;

    IF _deal_salesperson_id IS NULL THEN
      RAISE EXCEPTION 'مسئول معامله برای لینک پیش‌فاکتور مشخص نیست.' USING ERRCODE = '22023';
    END IF;

    _interaction_id := _deal_id;
    _quote_salesperson_id := _deal_salesperson_id;
  END IF;
  ...
  INSERT INTO public.sales_quotes (
    ...
    visitor_id, warehouse_id, interaction_id,
    ...
    _quote_salesperson_id, '', p_settlement_type_id, p_customer_id,
    _went_below,
    ...
    _credit_snapshot,
    p_visitor_id, _wh, _interaction_id,
