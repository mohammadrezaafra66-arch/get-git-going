-- 212 — Fix NULL bypasses in sales quote exception enforcement.
-- Scope: LAN/local application migration. Do not apply to production without the same reviewed rollout process.
-- Root cause: PostgreSQL `NULL <> 'expected'` evaluates to NULL, so IF bodies were skipped.
-- Preserves: function signature, SECURITY DEFINER, search_path, grants, audit/snapshot behavior,
-- and Requirement 215 stock behavior (creation does not validate stock; finalization does).
