-- 356 — one cheque is consumed once (Gate A phase 3, BLOCKER B1)
--
-- ==============================================================================================
-- THE DEFECT
-- ==============================================================================================
--
-- Migration 354's uniqueness rule excluded status = 'rejected', and create_payment's own EXISTS
-- guard carried the same exclusion. Rejecting a voucher therefore FREED its cheque — while the
-- journal entry that voucher had posted stayed status='posted' and immutable (343), with no
-- reverse_document to withdraw it (OG-14). Re-endorsing credited the SAME cheque a second time.
--
-- 354's own header presented this as a feature: "an operator who rejects a mistaken endorsement has
-- to be able to endorse it correctly." The documented correction path WAS the double-count.
--
-- Reproduced by Gate A, real invocations under an admin JWT inside BEGIN … ROLLBACK:
--
--   one 300,000 cheque (RCP-1405-000052)
--     -> endorse to supplier A            PAY-1405-000052   posted
--     -> UPDATE ... SET status='rejected' UPDATE 1
--        rejected voucher entry still posted | posted_entries = 1
--     -> endorse to supplier B            PAY-1405-000053   SUCCEEDS
--   POSTED entries against this ONE cheque | n = 2 | total_cheque_receivable_credited = 600000
--
-- Two suppliers each had 300,000 debited to their payable against a single 300,000 cheque, and
-- neither entry can ever be removed.
--
-- ==============================================================================================
-- THE OWNER'S DECISION — 2026-08-19, option (a): UNCONDITIONAL
-- ==============================================================================================
--
-- One cheque is consumed once. The cost is accepted and is recorded here plainly rather than
-- softened: UNTIL reverse_document EXISTS, A MISTAKEN ENDORSEMENT CANNOT BE CORRECTED. That cheque
-- stays consumed. Refusing a correction is strictly safer than silently permitting a permanent
-- double credit.
--
-- Consequence that follows from the decision (not itself a decision): OG-14 was scheduled as "must
-- close before phase 9". Option (a) makes every endorsement error permanent in the meantime, so
-- OG-14 becomes reachable-urgent the moment anyone uses this path. reverse_document is NOT built
-- here — it needs its own dispatch. The changed urgency is recorded in 00-progress.md against OG-14.
--
-- Because the refusal is now permanent, the Persian message tells the user WHY it cannot be undone
-- rather than leaving them to guess. That is the only user-visible change beyond the refusal itself.
--
-- ==============================================================================================
-- WHAT WRITES OR DEPENDS ON WHAT I AM CHANGING  (README-EXECUTION §H, first half)
-- ==============================================================================================
--
--   Writers of payment_vouchers.status: NO SQL function writes 'rejected'. Measured — the only two
--   functions that write the table are create_payment (writes 'approved') and
--   pay_purchase_with_voucher (writes 'approved'). 'rejected' is reachable only by a direct UPDATE
--   through PostgREST under payment_vouchers_update_finance, which grants UPDATE to admin and
--   accountant. There is no UI control for it today.
--
--   Readers of endorsed_receipt_id: create_payment ONLY. No view, no trigger, and nothing in src/.
--
--   ROWS THAT WOULD BLOCK THIS MIGRATION: the index is becoming STRICTER, so any cheque already
--   referenced by more than one voucher would prevent it being built. Measured before writing:
...
--   There is nothing to migrate over.
--
-- WHAT WILL READ THE ROWS THIS AFFECTS (§H, second half): unchanged from 355. This migration
-- narrows what may be created; it changes no column, no classification and no export behaviour.
--
-- ==============================================================================================
-- WHY create_payment IS REPLACED IN FULL
-- ==============================================================================================
--
-- Only two things change inside it: the EXISTS predicate loses "AND pv.status <> 'rejected'", and
-- the message gains the explanation. The body below was generated FROM THE LIVE DEFINITION
-- (pg_get_functiondef, CLAUDE.md rule 6 — the live definition is the truth, the repo file may be
-- stale) with exactly that edit applied, so what is deployed and what is reviewed are the same
-- text. The signature is byte-identical, so this REPLACES rather than overloads (rule 5).
--
-- Changing only the index would leave the constraint and the Persian-message path disagreeing: the
-- user would be refused by a raw unique-violation instead of a sentence. Both move together.
--
-- Rollback: docs/verification/356-down.sql — statements only, with a pre-flight gate. It restores
-- the previous behaviour faithfully, which means it REOPENS B1; the file says so.
...
COMMENT ON INDEX public.payment_vouchers_endorsed_receipt_unique_idx IS
  'One cheque is consumed once. UNCONDITIONAL since migration 356 (Gate A phase 3, B1): the '
  'previous partial form excluded status=''rejected'', which freed a cheque whose posted journal '
  'entry remained immutable, allowing the same cheque to be credited twice. Owner decision '
  '2026-08-19 option (a); the accepted cost is that a mistaken endorsement cannot be corrected '
  'until reverse_document exists (OG-14).';

-- ------------------------------------------------------- create_payment, in full ----
...
BEGIN
  ...
  ELSE  -- cheque, endorsed (task 3.8)
    ...
    IF EXISTS (SELECT 1 FROM public.payment_vouchers pv
                WHERE pv.endorsed_receipt_id = p_endorsed_cheque_id) THEN
      RAISE EXCEPTION 'این چک قبلاً ظهرنویسی شده است و دوباره قابل استفاده نیست؛ تا زمانی که امکان صدور سند برگشتی فراهم نشده، ظهرنویسی اشتباه قابل اصلاح نیست'
        USING ERRCODE = 'P0001';
    END IF;
