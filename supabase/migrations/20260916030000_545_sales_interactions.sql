...
-- ============================================================================
-- 545 - public.sales_interactions: CRM touchpoints for the sales desk.
-- ============================================================================
--
-- Forward: create table + RLS + indexes + register persons FK in person_merge.
-- Reverse: docs/research/sales-desk-9-needs/orchestration/545-down.sql
--
-- CLAUDE.md rule 9 / migration 328: person_merge registry key FIRST, then CREATE TABLE
-- with the persons FK inline. Live person_merge body read via pg_get_functiondef before
-- this file was written; only the sales_interactions.person_id registry line is added.
--
-- Notify of salesperson assignment is migration 547 (trigger), not here.
-- ============================================================================

SET lock_timeout = '60s';

-- 1. person_merge with sales_interactions.person_id registered
...
CREATE TABLE IF NOT EXISTS public.sales_interactions (
  ...
  salesperson_id uuid NULL REFERENCES public.profiles(id),
  author_id uuid NOT NULL REFERENCES public.profiles(id),
  call_log_id uuid NULL REFERENCES public.call_logs(id),
  next_follow_up_at timestamptz NULL,
  followed_up_at timestamptz NULL,
  source text NOT NULL DEFAULT 'manual',
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT sales_interactions_kind_check
    CHECK (kind = ANY (ARRAY['request'::text, 'call'::text, 'note'::text])),
  CONSTRAINT sales_interactions_status_check
    CHECK (status = ANY (ARRAY['open'::text, 'won'::text, 'lost'::text, 'cancelled'::text, 'done'::text]))
);

COMMENT ON TABLE public.sales_interactions IS
  'Sales-desk touchpoints: request / call / note linked to a person (and optional customer). Migration 545.';

CREATE INDEX IF NOT EXISTS sales_interactions_person_id_idx
  ON public.sales_interactions (person_id);
CREATE INDEX IF NOT EXISTS sales_interactions_salesperson_id_idx
  ON public.sales_interactions (salesperson_id);
CREATE INDEX IF NOT EXISTS sales_interactions_next_follow_up_at_idx
  ON public.sales_interactions (next_follow_up_at)
  WHERE next_follow_up_at IS NOT NULL;
CREATE INDEX IF NOT EXISTS sales_interactions_status_kind_idx
  ON public.sales_interactions (status, kind);
CREATE INDEX IF NOT EXISTS sales_interactions_call_log_id_idx
  ON public.sales_interactions (call_log_id)
  WHERE call_log_id IS NOT NULL;

-- ----------------------------------------------------------------------------
-- 3. updated_at (shared project pattern)
-- ----------------------------------------------------------------------------
DROP TRIGGER IF EXISTS trg_sales_interactions_updated_at ON public.sales_interactions;
CREATE TRIGGER trg_sales_interactions_updated_at
  BEFORE UPDATE ON public.sales_interactions
  FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();

-- ----------------------------------------------------------------------------
-- 4. RLS
-- Staff roles: sales / admin / manager / accountant.
-- SELECT: admin/manager see all; others see author, assignee, or customers they own.
-- INSERT: staff with author_id = auth.uid().
-- UPDATE: author OR salesperson OR admin/manager.
-- ----------------------------------------------------------------------------
...
DROP POLICY IF EXISTS sales_interactions_insert_staff ON public.sales_interactions;
CREATE POLICY sales_interactions_insert_staff ON public.sales_interactions
...
-- ----------------------------------------------------------------------------
-- 5. Grants (anon closed; authenticated CRUD via RLS)
-- ----------------------------------------------------------------------------
REVOKE ALL ON TABLE public.sales_interactions FROM PUBLIC;
REVOKE ALL ON TABLE public.sales_interactions FROM anon;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE public.sales_interactions TO authenticated;
GRANT ALL ON TABLE public.sales_interactions TO service_role;
...
BEGIN
  ...
  IF _bad <> 0 THEN
    RAISE EXCEPTION '545: % person FK columns disagree with person_merge registry', _bad;
  END IF;
  ...
  IF _keys <> _fks THEN
    RAISE EXCEPTION '545: registry has % keys against % FKs', _keys, _fks;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM public.person_merge_registry_keys()
     WHERE registry_key = 'sales_interactions.person_id'
  ) THEN
    RAISE EXCEPTION '545: sales_interactions.person_id missing from registry extractor';
  END IF;

  SELECT string_agg(p, ', ') INTO _anon
    FROM unnest(ARRAY['SELECT', 'INSERT', 'UPDATE', 'DELETE',
                      'TRUNCATE', 'REFERENCES', 'TRIGGER']) p
   WHERE has_table_privilege('anon', 'public.sales_interactions', p);
  IF _anon IS NOT NULL THEN
    RAISE EXCEPTION '545: anon still holds % on sales_interactions', _anon;
  END IF;

  IF NOT (SELECT relrowsecurity FROM pg_class WHERE oid = 'public.sales_interactions'::regclass) THEN
    RAISE EXCEPTION '545: RLS not enabled on sales_interactions';
  END IF;

  IF (SELECT count(*) FROM pg_policy WHERE polrelid = 'public.sales_interactions'::regclass) < 3 THEN
    RAISE EXCEPTION '545: expected at least 3 policies on sales_interactions';
  END IF;

  RAISE NOTICE '545 OK: sales_interactions created; % person FKs registered; anon closed', _fks;
END
$do$;
