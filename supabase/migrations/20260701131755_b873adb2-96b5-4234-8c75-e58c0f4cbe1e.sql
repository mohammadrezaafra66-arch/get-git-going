...
BEGIN
  ...
  -- Create per-salesperson customer temp table ONCE outside the loop; TRUNCATE per iteration
  ...
  UPDATE _cust_alloc SET
    ...
    binding_constraint = CASE
      WHEN has_overdue THEN 'overdue_block'
      WHEN credit_limit IS NOT NULL AND raw_allocation > credit_limit THEN 'credit_limit'
      ELSE 'formula'
    END
  WHERE true;
  ...
  INSERT INTO public.customer_capital_allocations_dynamic(
    ...
    raw_allocation, credit_limit, has_overdue, has_profile, final_limit, binding_constraint
  )
