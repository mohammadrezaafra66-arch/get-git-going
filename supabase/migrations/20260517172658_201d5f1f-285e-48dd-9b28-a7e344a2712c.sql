-- AFRA-20260517-PERSONS-U01-S18A
-- Atomic customer ↔ person link helpers. SECURITY INVOKER — RLS preserved.
...
COMMENT ON FUNCTION public.customer_set_person(uuid, uuid, text) IS
  'AFRA S18A — Atomically set customers.person_id and create/refresh the matching active person_context_links row. SECURITY INVOKER: caller must satisfy customers UPDATE RLS and person_context_links INSERT RLS (admin/manager).';
...
COMMENT ON FUNCTION public.customer_clear_person(uuid, text) IS
  'AFRA S18A — Atomically clear customers.person_id and close the matching active person_context_links row(s). SECURITY INVOKER: RLS preserved. No DELETE.';
