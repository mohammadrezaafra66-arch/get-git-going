-- Phase 7 (ادامه) — شمردن «تبلیغ‌های انجام‌شده» در امتیاز کارمند (۱۶۷/۱۶۸)
--
-- تنها تغییرِ `calculate_employee_score` نسبت به نسخهٔ قبلی سه چیز است:
--   ۱) چهار متغیر شمارنده `_promo_d/_promo_w/_promo_m/_promo_t`
--   ۲) یک SELECT که رویدادهای `promotion_completed` را از
--      `employee_score_events` برای بازه‌های روز/هفته/ماه/کل می‌شمارد
--   ۳) یک شاخهٔ CASE برای کلید KPI جدید `promotions_completed`
-- بقیهٔ بدنه دقیقاً همان قبلی است؛ محاسبهٔ هیچ KPI موجودی عوض نمی‌شود.
