-- 415: a pre-invoice line must name a product that exists in the system.
--
-- WHY
--   sales_quote_items.source accepted three values and only 'product_price' required a
--   product_id. 'manual' and 'quick_price' both let a salesperson type a name and a price for
--   goods that were never defined -- the same hole under two labels; the two UI tabs behind
--   them shared one component and differed by a hint string. Stock, pricing and the catalogue
--   all key off product_id, so a line without one is invisible to every one of them.
--
--   The owner's rule: only accounting creates products. A salesperson who needs a new item
--   asks accounting for it. Closing this in the UI alone would have been frontend-only
--   authorisation -- CLAUDE.md rule 6 -- because anyone could call the RPC directly.
--
-- WHAT IS DELIBERATELY NOT DONE
--   The sales_quote_item_source ENUM is left intact, with all three values. Narrowing it
--   would break the one existing 'manual' row (SQ-2026-000001, rejected, 2026-07-19) and every
--   read of it. product_id is likewise left nullable for the same reason. The guard applies to
--   new writes only; old quotes stay readable exactly as they are.
--
-- SAFETY: CREATE OR REPLACE of one function. No DROP, no DELETE, no schema change, no data
-- change. The only new behaviour is a rejection that did not happen before.
...
DECLARE
  v_src_ok boolean;
BEGIN
  SELECT pg_get_functiondef(p.oid) LIKE '%این کالا در سیستم تعریف نشده است%'
    INTO v_src_ok
    FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public' AND p.proname = 'create_sales_quote_with_items';

  IF NOT COALESCE(v_src_ok, false) THEN
    RAISE EXCEPTION '415: the guard is not present in create_sales_quote_with_items';
  END IF;

  -- The enum must still carry all three values, or old rows become unreadable.
  IF (SELECT count(*) FROM pg_enum e JOIN pg_type t ON t.oid = e.enumtypid
       WHERE t.typname = 'sales_quote_item_source') <> 3 THEN
    RAISE EXCEPTION '415: the source enum was narrowed -- existing rows would break';
  END IF;

  -- And the pre-existing free item must still be there, untouched.
  ...
  RAISE NOTICE '415: quote lines now require a real product; history left intact';
END
$verify$;
