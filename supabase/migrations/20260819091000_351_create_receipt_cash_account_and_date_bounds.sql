-- 351 — create_receipt: the cash channel needs a cash box, and a receipt needs a plausible date
--
-- Gate A phase 2: defect B1 (second half) and defect M6.
-- docs/execution/phase-2-GATE-A.md.
--
-- This is a CREATE OR REPLACE of the function migration 349 installed. The live definition was
-- read with pg_get_functiondef before this file was written (CLAUDE.md rule 4) and confirmed
-- byte-identical to 20260818181000_349_create_receipt.sql — md5 of pg_get_functiondef =
-- c9d9498427cd32612e471c9d447cef18, length 19407 — so this migration is that file plus exactly
-- three edits, and nothing else changed by accident:
--
--   1. DECLARE gains _account_type text.
--   2. Section 2 gains two date bounds (M6).
--   3. Section 4 gains the channel/account-type agreement check (B1 second half).
--
-- The signature is UNCHANGED. This is a replacement, not an overload — CLAUDE.md rule 5 does not
-- apply, and no DROP FUNCTION is needed or wanted. CREATE OR REPLACE preserves the ACL, so the
-- grants issued by 349 (authenticated, service_role; no PUBLIC, no anon) survive; verified by
-- reading pg_proc.proacl back after applying.
--
-- WHAT READS WHAT I AM ABOUT TO CHANGE:
--
--   * Nothing in the application. `grep -rn create_receipt src/ server/` returns no call site —
--     the legacy PaymentReceiptForm.tsx path survives until task 6.9 (D12). PostgREST exposes the
--     function; the signature and RETURNS TABLE are unchanged, so the exposed shape is identical
--     and src/integrations/supabase/types.ts does not need regenerating.
--   * No other database function calls it: a catalogue scan of every public function body for
--     'create_receipt' returns only this function.
--
-- WHAT WRITES WHAT I AM ABOUT TO CHANGE: this function is the only writer of its own behaviour.
-- The rows it produces are read by asan_list_bank_deposit_export (narrowed by 350),
-- asan_list_journal_export, get_account_ledger, get_receivable_detail, vw_account_balances,
-- vw_customer_receivables, calculate_credit_score and the two employee-score triggers. This
-- migration only ever REFUSES inputs — it never writes a row shape 349 could not produce — so no
-- existing reader can start seeing something new, and no row already written becomes invalid.
--
-- BEHAVIOUR CHANGE THAT IS NOT A REGRESSION BUT LOOKS LIKE ONE:
--
-- Task 2.5's acceptance command ("a cash receipt created with no tracking_number succeeds and
-- stores a generated one") no longer passes on this database, because no bank_accounts row with
-- account_type='cash' exists yet. That is the intended outcome of B1's second half. The cash
-- branch is proved instead against a صندوق row created inside BEGIN … ROLLBACK; see
-- docs/execution/phase-2-REMEDIATION-PROGRESS.md, task R2.
--
-- Rollback: docs/verification/351-down.sql — statements only, no BEGIN/COMMIT (M7). It carries the
-- verbatim pg_get_functiondef output of the pre-351 definition, so it cannot drift.
...
COMMENT ON FUNCTION public.create_receipt(
  ...
  'Contract: docs/api/rpc-contracts.md section 1. Role gate admin/accountant/manager (OG-13). '
  'Cash requires a bank_accounts row with account_type=''cash''; bank requires account_type=''bank'' '
  '(migration 351, Gate A B1). p_payment_date may not be in the future, nor older than the '
  'previous Jalali year (migration 351, Gate A M6). Migration 349, phase 2.';
