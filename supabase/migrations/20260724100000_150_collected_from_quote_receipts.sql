-- =====================================================================
-- 150 - Phase 2.1: sales KPI collected component reads quote-linked receipts
-- =====================================================================
-- calculate_employee_score's collected amount was hardcoded 0 (migration 146,
-- when no quote linkage existed). It now sums APPROVED receipt allocations
-- against this salesperson's ACCEPTED quotes, within the existing 6-month
-- window, capped per quote at final_amount. The 0.8*collected + 0.2*issued
-- blend and everything else are unchanged. Recompute triggers that invoke this
-- function are already fixed (migrations 148, 149).
--
-- calculate_salesperson_collected_sales is a SEPARATE, invoice-based function
-- NOT called by scoring; left unchanged (out of scope for the minimum edit).
--
-- ROLLBACK: restore the prior body (collected := 0) from git history.
-- =====================================================================
