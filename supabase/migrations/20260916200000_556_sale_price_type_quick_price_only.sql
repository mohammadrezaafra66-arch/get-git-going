-- =========================================================
-- 556: Flag sale_price_types used only by /pricing/quick-price.
-- These types are excluded from sales search prices and from
-- normal sale-price-type selectors / publish.
-- =========================================================
SET client_encoding = 'UTF8';

ALTER TABLE public.sale_price_types
  ADD COLUMN IF NOT EXISTS is_quick_price_only boolean NOT NULL DEFAULT false;

COMMENT ON COLUMN public.sale_price_types.is_quick_price_only IS
  'اگر true باشد فقط در صفحه محاسبه سریع قیمت استفاده می‌شود و در جستجوی فروش، انتشار و لیست‌های قیمت عادی دیده نمی‌شود.';

-- Quick-price-only types must not enqueue the recompute queue (plan: skip publish+queue).
...
BEGIN
  IF TG_OP = 'DELETE' THEN
    ...
    v_spt := OLD.id;
    IF COALESCE(OLD.is_quick_price_only, false) THEN
      RETURN OLD;
    END IF;
  ELSE
    r := NEW;
    v_spt := NEW.id;
    IF COALESCE(NEW.is_quick_price_only, false) THEN
      RETURN NEW;
    END IF;
    IF TG_OP = 'UPDATE' THEN
      IF NEW.is_active IS NOT DISTINCT FROM OLD.is_active
         ...
         AND NEW.is_quick_price_only IS NOT DISTINCT FROM OLD.is_quick_price_only
      THEN
        RETURN NEW;
      END IF;
      -- Turning quick-price-only ON: no enqueue (nothing to publish).
      -- Turning it OFF while active: fall through and enqueue.
    END IF;
    v_reason := CASE
...
-- Basis: 20260722111653_130_sales_search_stock_sort.sql
-- Change: both price arms require COALESCE(spt.is_quick_price_only, false) = false
...
BEGIN
  ...
  with_prices AS (
    SELECT b.*,
      (
        SELECT jsonb_agg(x ORDER BY x_sort)
        FROM (
          -- baseline rows (settlement_type_id NULL) — exactly today's behavior
          SELECT jsonb_build_object(
            'sale_price_type_id', spt.id,
            'settlement_type_id', NULL,
            'code', spt.code,
            'title', spt.title,
            'settlement_title', NULL,
          ...
          ) AS x, (spt.sort_order * 1000) AS x_sort
          FROM sale_price_types spt
          LEFT JOIN product_computed_prices pcp
            ON pcp.product_id = b.id AND pcp.sale_price_type_id = spt.id
           AND pcp.settlement_type_id IS NULL
          WHERE spt.is_active = true
            AND COALESCE(spt.is_quick_price_only, false) = false
            AND (NOT (b.stock_status = 'unavailable' AND NOT v_is_privileged))

          UNION ALL

          -- per-settlement rows — only those with an actual computed price
          SELECT jsonb_build_object(
            'sale_price_type_id', spt.id,
            'settlement_type_id', st.id,
            'code', spt.code,
            'title', spt.title,
            'settlement_title', st.title,
            'sort_order', st.sort_order,
            'current_price', pcp2.rounded_sale_price,
            'previous_price', (
              SELECT h2.new_sale_price
              FROM product_sale_price_history h2
              WHERE h2.product_id = b.id
                AND h2.sale_price_type_id = spt.id
                AND h2.settlement_type_id = st.id
                AND h2.created_at < (
                  SELECT MAX(h3.created_at) FROM product_sale_price_history h3
                  WHERE h3.product_id = b.id AND h3.sale_price_type_id = spt.id
                    AND h3.settlement_type_id = st.id
                )
              ORDER BY h2.created_at DESC LIMIT 1
            ),
            'last_updated_at', (
          ...
          ) AS x, (spt.sort_order * 1000 + st.sort_order + 1) AS x_sort
          FROM sale_price_types spt
          CROSS JOIN settlement_types st
          JOIN product_computed_prices pcp2
            ON pcp2.product_id = b.id AND pcp2.sale_price_type_id = spt.id
           AND pcp2.settlement_type_id = st.id
          WHERE spt.is_active = true
            AND COALESCE(spt.is_quick_price_only, false) = false
            AND st.is_active = true
            AND pcp2.rounded_sale_price IS NOT NULL
            AND (NOT (b.stock_status = 'unavailable' AND NOT v_is_privileged))
        ) s
      ) AS prices_json,
