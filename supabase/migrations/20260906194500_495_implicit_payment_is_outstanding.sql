...
-- ============================================================================
-- 495 - H-2, corrected: the implicit amount IS the outstanding balance.
-- ============================================================================
--
-- 🔴 THIS CORRECTS MIGRATION 494, WHICH I APPLIED AN HOUR EARLIER. Rule 6 forbids
-- editing that file, so the correction is a new migration.
--
-- WHAT 494 GOT WRONG
-- ---------------------------------------------------------------------------
-- The mission brief's hazard H·j prescribed the mechanics as
-- "clamp _amt := LEAST(_amt, _outstanding) only when _amount IS NULL". That
-- formulation CONTRADICTS the owner decision it was meant to implement, which is
-- recorded in CONTRACTS.md section 2 as:
--
--   "When pay_purchase_with_voucher receives _amount IS NULL, the implicit amount
--    pays the OUTSTANDING balance, never cash_price."
--
-- LEAST(cash_price, outstanding) returns cash_price whenever cash_price is the
-- smaller of the two -- which is precisely "cash_price", the thing the decision
-- says never to pay. Measured on the live purchase ba1c75a0 (total_amount
-- 12,000,000,000, cash_price 10,000,000,000) WITH 494 applied:
--
--   click with no amount -> voucher 10,000,000,000  (= cash_price)
--                           outstanding 2,000,000,000, paid_at NULL
--
-- The brief's own required proof for this row says the opposite of that result:
-- "click with no amount -> outstanding 0, paid_at set". Both the owner decision
-- and the required proof therefore say the implicit amount IS the outstanding
-- balance, and the LEAST() formulation was the brief's own suggested mechanics
-- rather than the decision. The decision wins.
--
-- The clamp still applies ONLY when _amount IS NULL, so an EXPLICIT over-payment
-- remains an error rather than being silently reduced. The _debt <= 0 arm keeps
-- the pre-478 behaviour for a purchase with no usable total. Signature unchanged
-- (11 args) -- rule 5.
-- ============================================================================
...
BEGIN
  ...
  -- (CLAUDE.md rule 5).
  -- 495 CORRECTION to 494. 494 used LEAST(_amt, _outstanding), which still pays
  -- cash_price whenever cash_price < outstanding -- exactly what the owner decision
  -- forbids. Measured on ba1c75a0 (total 12bn, cash_price 10bn) WITH 494 applied:
  --   click with no amount -> paid 10,000,000,000 (cash_price), outstanding 2bn,
  --                           paid_at still NULL.
  -- The owner decision is "the implicit amount pays the OUTSTANDING balance, NEVER
  -- cash_price" (CONTRACTS.md section 2), so the implicit amount IS the outstanding
  -- balance -- not the smaller of it and cash_price.
