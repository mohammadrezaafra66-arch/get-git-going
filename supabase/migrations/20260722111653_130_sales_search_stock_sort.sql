-- 130 — Quick Sales Search: order results by stock status, then name.
--
-- With the stock filter set to «همه», results are grouped so in-stock products
-- surface first: available → limited → unknown → unavailable, and alphabetically
-- by product name within each group. LIMIT/OFFSET stay AFTER this ORDER BY, so
-- pagination is correct AND the first N results favor in-stock items.
--
-- Basis: this is a faithful CREATE OR REPLACE of the live definition, which was
-- verified byte-for-byte identical to 20260718195703_sales_search_per_settlement_prices.sql
-- (no drift). The ONLY change vs. that base is the `base` CTE's ORDER BY
-- (previously `ORDER BY p.name ASC`). No signature/behavior/price logic changed.
