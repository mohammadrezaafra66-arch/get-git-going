...
-- ============================================================================================
-- 523 - migration 477, re-issued for production's ACTUAL table shape.
--       477 itself is NOT edited (CLAUDE.md rule 6). It is skipped on production and this
--       runs in its place.
--
-- ASCII-ONLY BY DESIGN, exactly as 477 is.
--
-- ============================================================================================
-- WHY THIS FILE EXISTS - measured on production, 2026-09-12
-- ============================================================================================
--
-- 477 aborted at order 39 of the production run:
--
--     ERROR:  relation "zz_retired_dynamic_parameter_weights_backup_142" does not exist
--
-- 477 is not a catalogue walk. It is a STATIC list of 390 REVOKE statements generated from the
-- TEST database's catalogue on 2026-09-06. On that database migrations 450 and 452 had already
-- run, and they RENAME six tables to zz_retired_*. So 477 names the six by their post-rename
-- names.
--
-- On production 450 and 452 were SKIPPED - each asserts an absolute row count read from the
-- test database, and each of those counts is wrong against production's data (450 has four
-- wrong constants, 452 two). The six tables therefore still carry their ORIGINAL names, the
-- six zz_retired_* relations do not exist, and 477 cannot apply.
--
-- 476 had already landed when 477 aborted. 477 rolled back whole, so anon_readable_relations
-- returned to 214 and no partial state was left behind.
--
-- ============================================================================================
-- WHAT THIS FILE IS, PRECISELY
-- ============================================================================================
--
-- This is 477 with EXACTLY TWELVE LINES REMOVED and nothing else changed. The twelve are the
-- six zz_retired_* tables, each appearing twice - once in the write-revoke block and once in
-- the SELECT-revoke block:
--
--     zz_retired_dynamic_parameter_weights_backup_142        (renamed by 452)
--     zz_retired_dynamic_parameter_weights_backup_20260722   (renamed by 452)
--     zz_retired_knowledge_articles                          (renamed by 450)
--     zz_retired_messages                                    (renamed by 450)
--     zz_retired_price_list_items                            (renamed by 450)
--     zz_retired_price_lists                                 (renamed by 450)
--
-- Every other REVOKE below is byte-identical to 477, including the eleven tables that KEEP
-- SELECT and the reasoning that derived them. 477 targets 203 distinct tables; this targets
-- 197. Verify with:
--
--     diff <(grep '^REVOKE' 477.sql) <(grep '^REVOKE' 523.sql)
--     -- expect exactly 12 deletions, zero additions, zero modifications
--
-- ============================================================================================
-- WHAT THIS FILE DELIBERATELY DOES NOT DO
-- ============================================================================================
--
-- It does NOT revoke anon from the six tables under their original names, and it does NOT
-- touch payment_receipts_backup_20260722 - which 450 DROPPED on the test database before 477
-- was generated, so 477 never contained a line for it either.
--
-- Those seven tables keep their anon grants after this migration. That is a real gap and it is
-- NOT closed here, because closing it means adding REVOKE statements that 477 never had, and
-- this file's whole value is being provably 477-minus-twelve-lines. Migration 524 closes it
-- separately, driven by the catalogue rather than by a static list, so it cannot fail the way
-- 477 just did.
--
-- ============================================================================================
-- THE ORIGINAL HEADER OF 477 FOLLOWS, VERBATIM
-- ============================================================================================
-- It is retained in full because the derivation of the eleven-table keep list - four
-- independent methods, plus a live PostgREST probe of all 199 readable tables - is the
-- justification for every REVOKE in this file, and that justification is unchanged.
-- ============================================================================================
