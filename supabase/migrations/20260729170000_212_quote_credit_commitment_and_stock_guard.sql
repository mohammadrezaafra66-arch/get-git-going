-- 212 — Strict quote credit/overdue/accounting exception flow + stock guard.
--
-- This migration is intentionally NOT applied by Codex. Apply on LAN only
-- after backup/review. It replaces create_sales_quote_with_items because the
-- enforcement must be server-side and atomic with quote creation.
--
-- Rollback note:
--   1) DROP FUNCTION public.create_sales_quote_with_items(
...
--      );
--   2) Re-apply migration 20260728203000_223_visitors.sql function body.
--   3) The added nullable sales_quotes columns may be left in place, or dropped
--      only after confirming no rows use them.
SET client_encoding='UTF8';

ALTER TABLE public.sales_quotes
  ADD COLUMN IF NOT EXISTS quote_exception_type text,
  ADD COLUMN IF NOT EXISTS quote_exception_confirmed_at timestamptz,
  ADD COLUMN IF NOT EXISTS quote_exception_confirmed_by uuid,
  ADD COLUMN IF NOT EXISTS quote_exception_minutes integer,
  ADD COLUMN IF NOT EXISTS quote_exception_amount numeric,
  ADD COLUMN IF NOT EXISTS quote_exception_text text,
  ADD COLUMN IF NOT EXISTS quote_exception_snapshot jsonb;

ALTER TABLE public.sales_quotes
  DROP CONSTRAINT IF EXISTS sales_quotes_exception_type_check;

ALTER TABLE public.sales_quotes
  ADD CONSTRAINT sales_quotes_exception_type_check
  CHECK (
    quote_exception_type IS NULL OR quote_exception_type IN (
...
COMMENT ON COLUMN public.sales_quotes.quote_exception_type IS
  'نوع مسیر غیرعادی صدور پیش‌فاکتور: تعهد معوقه، تعهد کسری اعتبار، یا تأیید حسابداری (۲۱۲).';
COMMENT ON COLUMN public.sales_quotes.quote_exception_text IS
  'متن تعهدی که فروشنده در لحظه صدور پیش‌فاکتور پذیرفته است (۲۱۲).';
COMMENT ON COLUMN public.sales_quotes.quote_exception_snapshot IS
  'اسنپ‌شات دلیل توقف و مسیر مجاز انتخاب‌شده هنگام صدور پیش‌فاکتور (۲۱۲).';
...
DROP FUNCTION IF EXISTS public.create_sales_quote_with_items(
  text, text, text, timestamp with time zone, numeric, numeric, numeric, jsonb,
  uuid, uuid, boolean, numeric, boolean, uuid
);
...
CREATE OR REPLACE FUNCTION public.create_sales_quote_with_items(
  ...
  p_quote_exception_type text DEFAULT NULL::text,
  p_quote_exception_minutes integer DEFAULT NULL::integer,
  p_quote_exception_amount numeric DEFAULT NULL::numeric,
  p_quote_exception_text text DEFAULT NULL::text
)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF p_customer_id IS NULL THEN
    IF p_quote_exception_type <> 'accounting_approval' THEN
  ...
  ELSE
    ...
    IF COALESCE(_credit.has_overdue, false) OR COALESCE(_credit.binding_constraint, '') = 'overdue' THEN
      IF p_quote_exception_type <> 'overdue_salesperson_commitment' THEN
    ...
    ELSE
      IF p_quote_exception_type <> 'credit_shortfall_salesperson_commitment' THEN
        RAISE EXCEPTION
