...
-- 421 — the guest branch's refusal message told the salesperson to get accounting approval.
--       Since 420 that is no longer the only way through, so the sentence was simply wrong.
--
-- WHAT CHANGED: one string literal. The condition, the ERRCODE, the signature, the owner,
-- SECURITY DEFINER, search_path and the GRANTs are untouched — the body below is
-- pg_get_functiondef() output captured after 420 with exactly one replacement applied.
-- Complete diff: docs/release/421-function-diff.txt
-- Rollback: docs/release/421-rollback-create_sales_quote_with_items.sql
--
-- The COMMITMENT text itself is NOT touched. This is the refusal message the server raises when
-- no route was chosen; the words a salesperson signs are a different string and stay verbatim.
