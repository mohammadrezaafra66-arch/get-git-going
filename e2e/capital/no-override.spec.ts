...
test.describe("D8-1 — the computed capital ceiling is read-only", () => {
  ...
  test("the override parameters are gone from the save RPC", async () => {
    // Calling with an override argument must not silently succeed.
    ...
    expect(res.status).toBeGreaterThanOrEqual(400);
    if (res.status >= 400 && errMessage(res.body)) {
      expect(errMessage(res.body).length).toBeGreaterThan(0);
    }
  });
});
