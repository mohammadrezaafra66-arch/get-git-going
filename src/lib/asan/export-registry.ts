 ...
 *
 * Entries flipped from `notBuiltYet` to real definitions as phases 4.3–4.7 landed; 367 added a
 * seventh (purchase payments and settlements). `notBuiltYet` stays in `export-types.ts`
...
export const ASAN_EXPORTS: Record<AsanExportKey, AsanExportDefinition> = {
  ...
  // M4.6 — filters over one builder, not separate mappers.
  receipts: RECEIPTS_EXPORT,
  payments: PAYMENTS_EXPORT,
  third_party: THIRD_PARTY_EXPORT,
  purchase_settlement: PURCHASE_SETTLEMENT_EXPORT,
