...
/**
 * کد ترم تسویهٔ «پیش واریز» در `settlement_types`.
 *
 * روی هر دو سرور عنوانش «پیش واریز(نقدی)» است — نه «پیش واریز» خالی — پس هرگز
 * روی عنوان تطبیق نده. عنوان از رابط کاربری قابل ویرایش است و `code` نیست، پس
 * تطبیق روی `code` تنها راهی است که با تغییر نام کاربر بی‌صدا نمی‌شکند.
 *
 * توجه: این با `BASE_SALE_PRICE_TYPE_CODE` یکی نیست. آن یک **نوع‌قیمت** است
 * (نقدی/چکی/همکاری) و این یک **ترم تسویه**. قیمت‌ها روی سه‌تایی
 * (محصول، نوع‌قیمت، ترم تسویه) ذخیره می‌شوند و این دو بُعد مستقل‌اند.
 *
 * ردیفی که `settlement_type_id IS NULL` دارد «قیمت پایه» است — همان چیزی که
 * قبل از افزوده‌شدن بُعد تسویه (۲۰۲۶-۰۷-۱۸) تنها قیمت موجود بود. محصولاتی که از
 * آن تاریخ به بعد دوباره منتشر نشده‌اند فقط ردیف پایه دارند، پس هر مصرف‌کننده‌ای
 * باید قاعدهٔ برگشت به پایه داشته باشد.
 */
export const PREPAY_SETTLEMENT_CODE = "cash";
