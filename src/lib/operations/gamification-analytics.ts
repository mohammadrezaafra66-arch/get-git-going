...
export async function getActiveSeason() {
  ...
  return (
    (data as Array<{ id: string; title_fa: string; starts_at: string; ends_at: string }>)[0] ?? null
  );
}
...
export async function getAchievementAnalytics(
  r: AnalyticsRange,
): Promise<AchievementAnalyticsRow[]> {
