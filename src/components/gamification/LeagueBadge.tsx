...
export function LeagueBadge({ tier, size = "sm", label = false, animated = false, className = "" }: LeagueBadgeProps) {
  const spec = (tier && (TIERS as Record<string, TierSpec>)[tier as string]) || null;
  if (!spec) {
    return (
      <div className={`inline-flex flex-col items-center gap-1 ${className}`}>
        <div className={`${SIZES[size].box} rounded-full border border-dashed border-muted-foreground/40 bg-muted/40 flex items-center justify-center`}>
  ...
  return (
    <div className={`inline-flex flex-col items-center gap-1 ${className}`}>
      ...
      <div
        ...
        style={{ boxShadow: `0 6px 20px -4px ${spec.glow}, 0 0 0 1px rgba(255,255,255,0.15) inset` }}
        aria-label={`League: ${spec.fa}`}
      >
        <div className={`relative h-full w-full rounded-[24%] bg-gradient-to-br ${spec.body} flex items-center justify-center overflow-hidden`}>
          {/* shine overlay */}
      ...
      {label ? <span className={`text-[11px] font-semibold ${size === "xs" ? "hidden" : ""}`}>{spec.fa}</span> : null}
    </div>
  );
}
