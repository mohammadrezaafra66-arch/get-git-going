...
import {
  MOODS, STATUS_LABELS, FOLLOW_UP_OPTIONS,
  updateEntryStatus, updateManagerNote, type MoodEntry, type EntryStatus,
  ...
  return (
    <Dialog open onOpenChange={(o) => { if (!o) onClose(); }}>
      <DialogContent dir="rtl" className="max-w-2xl max-h-[85vh] overflow-y-auto">
        <DialogHeader><DialogTitle>جزئیات ثبت حال‌وهوا</DialogTitle></DialogHeader>
        <div className="space-y-4 text-sm">
          <div className="flex items-center gap-3">
            <span className="text-3xl">{moodMeta?.emoji}</span>
            <div>
              <p className="font-semibold">{entry.mood_label}</p>
              <p className="text-xs text-muted-foreground">{entry.mood_date} • {new Date(entry.created_at).toLocaleString("fa-IR")}</p>
            </div>
            <Badge variant="outline" className="ms-auto">{STATUS_LABELS[entry.status]}</Badge>
          </div>

          {entry.reasons.length > 0 && (
            <div><p className="text-xs text-muted-foreground mb-1">دلایل</p><div className="flex flex-wrap gap-1">{entry.reasons.map((r) => <Badge key={r} variant="secondary">{r}</Badge>)}</div></div>
          )}
            ...
            <div className="space-y-2">
              ...
              {entry.answers.map((a, i) => (
                <div key={i} className="rounded-md border p-2">
                  ...
                  <p>{Array.isArray(a.value) ? (a.value as string[]).join("، ") : String(a.value ?? "")}</p>
                </div>
              ))}
            </div>
          )}
          ...
          {entry.free_text && (
            <div><p className="text-xs text-muted-foreground mb-1">متن آزاد</p><p className="rounded-md border p-3 leading-loose whitespace-pre-line">{entry.free_text}</p></div>
          )}

          <div className="flex items-center gap-2 text-xs">
            <span className="text-muted-foreground">پیگیری:</span>
            <Badge>{FOLLOW_UP_OPTIONS.find((o) => o.value === entry.wants_follow_up)?.label ?? entry.wants_follow_up}</Badge>
          </div>
          ...
          <div className="border-t pt-3 space-y-3">
            ...
            <div className="space-y-1">
              ...
              <textarea value={note} onChange={(e) => setNote(e.target.value.slice(0, 2000))}
                className="w-full rounded-md border bg-card p-3 text-sm min-h-24" placeholder="فقط برای مدیران قابل مشاهده…" />
              <div className="flex items-center justify-between">
                <span className="text-xs text-muted-foreground">{note.length}/2000</span>
                <Button size="sm" onClick={() => void saveNote()} disabled={busy}>ذخیره یادداشت</Button>
              </div>
            </div>
          </div>
        </div>
