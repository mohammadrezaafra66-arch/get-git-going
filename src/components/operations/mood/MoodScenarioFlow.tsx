...
export function MoodScenarioFlow({
  ...
  if (questions.length === 0)
    return <p className="text-muted-foreground text-sm">سؤالی برای این حال در دسترس نیست.</p>;
      ...
      <div className="grid sm:grid-cols-2 gap-2">
        {(opts as string[]).map((opt) => (
          <button
            ...
            className={cn(
              "rounded-xl border px-4 py-3 text-sm text-right transition-colors",
      ...
      <div className="flex gap-2 flex-wrap">
        {(opts as Array<{ value: number; label: string }>).map((o) => (
          <button
            ...
            className={cn(
              "rounded-lg border px-3 py-2 text-sm flex flex-col items-center min-w-16",
