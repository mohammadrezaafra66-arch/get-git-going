    ...
    >
      {q.isLoading ? (
        ...
        <p className="py-6 text-center text-sm text-muted-foreground">
          {isUp ? "کالایی با افزایش قیمت در این بازه پیدا نشد." : "کالایی با کاهش قیمت در این بازه پیدا نشد."}
        </p>
      ) : (
        <ul className="space-y-1.5">
          {q.data.map((p) => {
            ...
            return (
              <li key={`${p.product_id}-${p.sale_price_type_id}`} className="flex items-center gap-3 rounded-md border p-2 text-sm">
                <div className="min-w-0 flex-1">
                  <div className="truncate font-medium">{p.name}</div>
                  <div className="flex flex-wrap items-center gap-1.5 text-[11px] text-muted-foreground">
                    {p.brand?.name && <span>{p.brand.name}</span>}
                    <Badge variant="outline" className="h-4 px-1 text-[10px]">{p.sale_price_type_title}</Badge>
                  </div>
                </div>
                <div className="shrink-0 text-left">
                  <div className={`text-sm font-bold tabular-nums ${isUp ? "text-emerald-600" : "text-red-600"}`}>
                    {isUp ? "+" : ""}{formatNumber(pct)}٪
                  </div>
