...
import { ChevronDown, GitMerge, Loader2, X } from "lucide-react";
    ...
    <Collapsible open={open} onOpenChange={setOpen}>
      ...
      >
        <CollapsibleTrigger asChild>
          <button
            type="button"
            className="mb-0 flex w-full items-center gap-2 text-start"
            aria-expanded={open}
          >
            ...
            <span className="text-xs text-slate-500">
              {loading
                ? "در حال بررسی…"
                : pendingCount === 0
                  ? "موردی در صف نیست — فقط با تأیید شما"
                  : `${toFaDigits(pendingCount)} پیشنهاد — فقط با تأیید شما`}
            </span>
            <ChevronDown
              className={cn(
                "ms-auto h-4 w-4 shrink-0 text-slate-500 transition-transform",
                open && "rotate-180",
              )}
            />
          </button>
        ...
        {!open && !loading && pendingCount === 0 && (
          <p className="mt-2 text-xs text-slate-500">
            پیشنهاد ادغامی در صف نیست. برای جزئیات باز کنید.
          </p>
        )}
        ...
        <CollapsibleContent className="mt-3 space-y-3">
          ...
          <ul className="space-y-3">
            {suggestions.map((s) => {
              const keep = keepById[s.id] ?? s.source_item_id;
