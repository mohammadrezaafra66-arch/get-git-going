...
const ProductPriceChart = lazy(() => import("@/components/pricing/price-history/ProductPriceChart"));
...
export function BoardProductDetailsDrawer({ open, onOpenChange, productId, salePriceTypeId, salePriceTypeTitle }: Props) {
  ...
  const summary = useMemo(() => {
    ...
    const prev = previous ? Number(previous.new_sale_price) : (latest.old_sale_price !== null ? Number(latest.old_sale_price) : null);
  ...
  const currentUsd = mode === "usd" && latest ? tomanToUsd(Number(latest.new_sale_price), usdRate) : null;
            ...
            <Card>
              <CardContent className="grid grid-cols-2 gap-4 p-4">
                <div>
                  ...
                  {currentUsd !== null && (
                    <div className="text-xs text-muted-foreground" dir="ltr">≈ ${formatNumber(currentUsd)}</div>
                  )}
                </div>
                <div>
              ...
              <CardContent className="space-y-3 p-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="inline-flex rounded-md border border-border p-0.5">
                    <Button type="button" size="sm" variant={mode === "toman" ? "default" : "ghost"} className="h-7 px-2 text-xs" onClick={() => setMode("toman")}>تومان</Button>
                    <Button type="button" size="sm" variant={mode === "usd" ? "default" : "ghost"} className="h-7 px-2 text-xs" onClick={() => setMode("usd")}>دلار</Button>
                  </div>
                </div>
                <div className="flex flex-wrap gap-1">
                  {(["7d", "30d", "90d", "all"] as PriceRangeKey[]).map((r) => (
                    <Button key={r} type="button" size="sm" variant={range === r ? "default" : "outline"} className="h-7 px-2 text-xs" onClick={() => setRange(r)}>
                      {RANGE_LABEL[r]}
                    </Button>
                  ))}
                </div>
                {chartHistory.isLoading || (mode === "usd" && usdRateQuery.isLoading) ? (
                  ...
                  <Suspense fallback={<div className="flex h-[200px] items-center justify-center text-xs text-muted-foreground"><Loader2 className="ml-2 h-4 w-4 animate-spin" /></div>}>
                    <ProductPriceChart data={chartData} mode={mode} usdRate={usdRate} height={220} />
                  </Suspense>
                )}
                {mode === "usd" && usdRate && (
                  <div className="text-[11px] text-muted-foreground">
                    آخرین نرخ دلار: <Badge variant="outline" className="font-mono text-[10px]">{formatNumber(usdRate)} ت/$</Badge>
                  </div>
                )}
              </CardContent>
            </Card>
