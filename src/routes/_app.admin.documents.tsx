...
function AdminDocumentsPage() {
  ...
  const { data, isLoading, error } = useAllDocuments(filters);
  ...
  return (
    <div className="space-y-4" dir="rtl">
      <PageHeader title="مدیریت اسناد" description="بررسی و تأیید بیجک، فاکتور و حواله" />
            ...
            <div className="overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {rows.map((r) => (
                    <TableRow key={r.id}>
                      <TableCell>{documentTypeLabel(r.type)}</TableCell>
                      <TableCell
                      ...
                      <TableCell>
                        <Badge
                          variant="outline"
                          className={documentStatusBadgeClass(r.status)}
                        >
      ...
      <Dialog open={active !== null} onOpenChange={(o) => !o && setActive(null)}>
        <DialogContent dir="rtl" className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>
              {active ? `بررسی سند: ${active.file_name}` : ""}
            </DialogTitle>
          </DialogHeader>
          {active && (
            <div className="space-y-4">
              <DocumentCard document={active} />
              <Separator />
              <DocumentReviewActions document={active} />
            </div>
          )}
        </DialogContent>
      </Dialog>
