    ...
    handlers: {
      PATCH: async ({ request, params }) => {
        ...
        if (!isUuid(tableId) || !isUuid(rowId)) {
          logBotUsage({ api_key_id: null, table_id: null, endpoint, method: "PATCH",
            status_code: 400, error_code: "invalid_id", ip });
          return jsonResponse(400, { error: "invalid_id", message: "شناسه جدول یا ردیف نامعتبر است." });
        }
        ...
        const values =
          body && typeof body === "object" && !Array.isArray(body) && "values" in (body as Record<string, unknown>)
        ...
        const row = (Array.isArray(data) ? data[0] : data) as
          | { updated_count: number | null; applied_keys: string[] | null }
          | null;
        const applied = row?.applied_keys ?? [];

        logBotUsage({ api_key_id: auth.keyId, table_id: tableId, endpoint, method: "PATCH",
          status_code: 200, ip, request_size: raw.length, response_count: applied.length });
