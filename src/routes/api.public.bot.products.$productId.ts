  ...
  server: {
    handlers: {
      GET: async ({ request, params }) => {
        ...
        if (!isUuid(productId)) {
          logBotUsage({ api_key_id: null, table_id: null, endpoint, method: "GET", status_code: 400, error_code: "invalid_product_id", ip });
        ...
        if (!auth.ok) {
          logBotUsage({ api_key_id: null, table_id: null, endpoint, method: "GET", status_code: auth.status, error_code: auth.code, ip });
        ...
        if (!rl.ok) {
          logBotUsage({ api_key_id: auth.keyId, table_id: null, endpoint, method: "GET", status_code: 429, error_code: rl.code, ip });
        ...
        if (error) {
          ...
          if (/forbidden_product/.test(msg)) {
            ...
            message = "این کلید به این محصول دسترسی ندارد.";
          } else {
            const m = mapBotError(msg);
            code = m.code; status = m.status; message = m.message;
          }
          logBotUsage({ api_key_id: auth.keyId, table_id: null, endpoint, method: "GET", status_code: status, error_code: code, ip });
          return jsonResponse(status, { error: code, message });
        }

        if (!data) {
          logBotUsage({ api_key_id: auth.keyId, table_id: null, endpoint, method: "GET", status_code: 404, error_code: "product_not_found", ip });
          return jsonResponse(404, { error: "product_not_found", message: "محصولی با این شناسه یافت نشد." });
        }

        logBotUsage({ api_key_id: auth.keyId, table_id: null, endpoint, method: "GET", status_code: 200, ip, response_count: 1 });
