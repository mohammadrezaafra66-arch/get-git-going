...
export const Route = createFileRoute("/_app/admin/waybill-fields")({
  component: WaybillCustomFieldsAdminPage,
});
...
function WaybillCustomFieldsAdminPage() {
  ...
  const { data: rows, isLoading } = useQuery({
    enabled: allowed,
    queryKey: ["waybill-custom-fields", "all"],
    staleTime: 30_000,
  ...
  const save = async () => {
    ...
    if (editing) {
      const { error } = await supabase.from("waybill_custom_fields")
        .update(payload).eq("id", editing.id);
      err = error;
    } else {
      const { data, error } = await supabase.from("waybill_custom_fields")
    ...
    if (user?.id && savedId) {
      await supabase.from("audit_logs").insert({
        actor_id: user.id,
        entity_type: "waybill_custom_field",
        entity_id: savedId,
        action: editing ? "waybill_custom_field_updated" : "waybill_custom_field_created",
        diff: payload as never,
      });
    }
    toast.success("ذخیره شد");
    setOpen(false);
    qc.invalidateQueries({ queryKey: ["waybill-custom-fields"] });
  };
  ...
  const toggleActive = async (row: FieldRow) => {
    ...
    if (user?.id) {
      await supabase.from("audit_logs").insert({
        ...
        entity_type: "waybill_custom_field",
        entity_id: row.id,
        action: "waybill_custom_field_status_changed",
  ...
  return (
    <div dir="rtl" className="space-y-4">
      <PageHeader
        title="فیلدهای سفارشی بیجک"
        description="مدیریت فیلدهای اضافی نمایش‌داده‌شده در فرم بیجک"
        actions={
