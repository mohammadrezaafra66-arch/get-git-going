      ...
      POST: async ({ request }) => {
        ...
        if (raw.length > MAX_BODY_BYTES) {
          ...
          return jsonResponse(413, {
            ...
            message: "بدنه بیش از حد مجاز است.",
          });
        }
        ...
        const sourceUrlRaw =
          typeof body.source_product_url === "string" ? body.source_product_url.trim() : "";
        const sourceIdRaw =
          typeof body.source_product_id === "string" ? body.source_product_id.trim() : "";
        ...
        if (sourceProductUrl && sourceProductUrl.length > MAX_URL_LEN) {
          return jsonResponse(400, {
            ...
            message: "source_product_url طولانی‌تر از حد مجاز است.",
          });
        }
        if (sourceProductId && sourceProductId.length > MAX_ID_LEN) {
          return jsonResponse(400, {
            error: "invalid_payload",
            message: "source_product_id طولانی‌تر از حد مجاز است.",
          });
        }
        ...
        if (!sourceTitle) {
          ...
          return jsonResponse(400, {
            error: "invalid_payload",
            message: "source_title الزامی است.",
          });
        }
        if (sourceTitle.length > MAX_TITLE_LEN) {
          return jsonResponse(400, {
            error: "invalid_payload",
            message: "source_title بیش از حد طولانی است.",
          });
        }

        const normalized =
          typeof body.normalized_source_title === "string"
        ...
        const notes =
          typeof body.notes === "string" ? body.notes.trim().slice(0, MAX_NOTES_LEN) || null : null;
        ...
        if (error) {
          ...
          return jsonResponse(500, {
            error: "server_error",
            message: "خطای داخلی هنگام ثبت کاندید.",
          });
        }
