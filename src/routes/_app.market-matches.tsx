...
function MarketMatchesPage() {
  ...
  const [statusFilter, setStatusFilter] = useState<MatchStatus | "review_queue" | "all">("review_queue");
  ...
  return (
    <div dir="rtl" className="space-y-4 p-4 md:p-6">
      <PageHeader title="بررسی تطبیق محصولات بازار" description="مدیریت candidateهای ربات‌ها برای نگاشت به محصولات داخلی افراکالا" />
        ...
        <Table>
          ...
          <TableBody>
            {loading && rows.length === 0 ? (
              ...
              <TableRow><TableCell colSpan={8} className="text-center py-8 text-muted-foreground">رکوردی یافت نشد</TableCell></TableRow>
            ) : (
              rows.map((r) => (
                <TableRow key={r.id}>
                  <TableCell>{SOURCE_LABELS[r.source_name] ?? r.source_name}</TableCell>
                  <TableCell className="max-w-xs truncate" title={r.source_title}>{r.source_title}</TableCell>
                  <TableCell className="text-xs">
                    {r.source_product_url ? (
                      <a href={r.source_product_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-primary hover:underline">
                  ...
                  <TableCell>
                    <Badge variant={STATUS_VARIANT[r.match_status]}>{STATUS_LABELS[r.match_status]}</Badge>
                  </TableCell>
                  <TableCell>{r.confidence_score != null ? Number(r.confidence_score).toFixed(1) : "—"}</TableCell>
                  <TableCell className="max-w-xs truncate">{r.afrakala_product_name_snapshot ?? "—"}</TableCell>
                  <TableCell className="text-xs text-muted-foreground">
                    {r.last_seen_at ? new Date(r.last_seen_at).toLocaleString("fa-IR") : "—"}
                  </TableCell>
                  <TableCell>
                    <Button size="sm" variant="outline" onClick={() => setDetail(r)}>بررسی</Button>
                  </TableCell>
                </TableRow>
              ))
            )}
      ...
      <div className="flex items-center justify-between text-sm text-muted-foreground">
        ...
        <div className="flex items-center gap-2">
          <Button size="sm" variant="outline" disabled={page === 0 || loading} onClick={() => setPage((p) => Math.max(0, p - 1))}>قبلی</Button>
          <span>صفحه {page + 1} از {pageCount}</span>
          <Button size="sm" variant="outline" disabled={page + 1 >= pageCount || loading} onClick={() => setPage((p) => p + 1)}>بعدی</Button>
        </div>
      </div>
...
function MatchDetailDialog({ match, onClose, onChanged }: {
  match: MatchRow;
  onClose: () => void;
  onChanged: () => void;
}) {
  const [productId, setProductId] = useState<string>(match.afrakala_product_id ?? "");
  const [productLabel, setProductLabel] = useState<string>(match.afrakala_product_name_snapshot ?? "");
  const [productSearch, setProductSearch] = useState("");
  const dProductSearch = useDebounce(productSearch, 300);
  const [results, setResults] = useState<Array<{ id: string; name: string; sku: string | null; brand?: { name: string | null } | null; category?: { name: string | null } | null }>>([]);
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      if (!dProductSearch.trim()) { setResults([]); return; }
      setSearching(true);
      try {
  ...
  const approve = async () => {
    if (!productId) { toast.error("ابتدا یک محصول داخلی انتخاب کنید"); return; }
    setBusy(true);
  ...
  const reject = async () => {
    if (!rejectReason.trim()) { toast.error("دلیل رد الزامی است"); return; }
    setBusy(true);
  ...
  const disable = async () => {
    if (!disableReason.trim()) { toast.error("دلیل غیرفعال‌سازی الزامی است"); return; }
    setBusy(true);
      ...
      <DialogContent dir="rtl" className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="space-y-2 text-sm">
            <div><span className="text-muted-foreground">منبع:</span> {SOURCE_LABELS[match.source_name]}</div>
            <div><span className="text-muted-foreground">عنوان:</span> {match.source_title}</div>
            <div className="text-xs text-muted-foreground break-all">{match.normalized_source_title}</div>
            {match.source_product_url && (
              <div><a href={match.source_product_url} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">{match.source_product_url}</a></div>
            )}
            {match.source_product_id && <div className="font-mono text-xs">ID: {match.source_product_id}</div>}
            <div><span className="text-muted-foreground">وضعیت فعلی:</span> <Badge variant={STATUS_VARIANT[match.match_status]}>{STATUS_LABELS[match.match_status]}</Badge></div>
            <div><span className="text-muted-foreground">اطمینان:</span> {match.confidence_score != null ? Number(match.confidence_score).toFixed(1) : "—"}</div>
            {match.afrakala_product_name_snapshot && (
              <div><span className="text-muted-foreground">محصول فعلی:</span> {match.afrakala_product_name_snapshot}</div>
            )}
            {match.reject_reason && <div className="text-destructive text-xs">دلیل: {match.reject_reason}</div>}
          </div>
          ...
          <div className="space-y-2">
            ...
            />
            {searching && <div className="text-xs text-muted-foreground"><Loader2 className="inline h-3 w-3 animate-spin" /> در حال جستجو…</div>}
            {results.length > 0 && (
              <div className="max-h-48 overflow-y-auto rounded border">
                {results.map((p) => (
                  <button
                    key={p.id}
                    type="button"
                    onClick={() => { setProductId(p.id); setProductLabel(p.name); setResults([]); setProductSearch(""); }}
                    className="block w-full text-right p-2 text-sm hover:bg-muted border-b last:border-b-0"
                  >
                    <div className="font-medium">{p.name}</div>
                    <div className="text-xs text-muted-foreground">
                      {p.sku ?? "بدون SKU"} • {p.brand?.name ?? "بدون برند"} • {p.category?.name ?? "بدون دسته"}
                    </div>
                  </button>
                ))}
              </div>
        ...
        <div className="grid gap-4 md:grid-cols-3 border-t pt-4">
          ...
          <div className="space-y-2">
            <Input placeholder="دلیل رد (الزامی)" value={rejectReason} onChange={(e) => setRejectReason(e.target.value)} />
            <Button onClick={reject} disabled={busy || !rejectReason.trim()} variant="destructive" className="w-full">
              <XCircle className="h-4 w-4 ml-1" /> رد تطبیق
            </Button>
          </div>
          <div className="space-y-2">
            <Input placeholder="دلیل غیرفعال‌سازی" value={disableReason} onChange={(e) => setDisableReason(e.target.value)} />
            <Button onClick={disable} disabled={busy || !disableReason.trim()} variant="outline" className="w-full">
          ...
          <div className="max-h-60 overflow-y-auto space-y-1 text-xs">
            {events.length === 0 ? (
              ...
              <div key={ev.id} className="flex items-center gap-2 rounded border p-2">
                <span className="font-mono text-[10px] text-muted-foreground">{new Date(ev.created_at).toLocaleString("fa-IR")}</span>
                <Badge variant="outline">{ev.event_type}</Badge>
                <span>{ev.actor}</span>
                {ev.old_status && <span className="text-muted-foreground">{STATUS_LABELS[ev.old_status]} ← </span>}
                {ev.new_status && <span className="font-medium">{STATUS_LABELS[ev.new_status]}</span>}
              </div>
            ))}
          </div>
        </div>
