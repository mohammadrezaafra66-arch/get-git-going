...
import {
  EVENT_TYPES, EVENT_TYPE_FA,
  getActiveSeason, getEmployees, getSummary, getTrend, getTopEmployees,
  getKpiEffectiveness, getMissionAnalytics, getAchievementAnalytics,
  getLeagueDistribution, getRiskEmployees,
...
function StatCard({ label, value, icon: Icon, hint }: { label: string; value: string | number; icon: React.ComponentType<{ className?: string }>; hint?: string }) {
  return (
    <Card>
...
function AnalyticsPage() {
  ...
  const seasonQ = useQuery({ queryKey: ["analytics-active-season"], queryFn: getActiveSeason, staleTime: 60_000 });
  const employeesQ = useQuery({ queryKey: ["analytics-employees"], queryFn: getEmployees, staleTime: 60_000 });
  ...
  const summaryQ      = useQuery({ queryKey: ["g-an-summary", ...baseKey],            queryFn: () => getSummary(filters) });
  const trendQ        = useQuery({ queryKey: ["g-an-trend", ...baseKey],              queryFn: () => getTrend(filters) });
  const topQ          = useQuery({ queryKey: ["g-an-top", ...baseKey],                queryFn: () => getTopEmployees(filters) });
  const kpiEffQ       = useQuery({ queryKey: ["g-an-kpieff", ...rangeKeyArr],         queryFn: () => getKpiEffectiveness(range) });
  const missionsQ     = useQuery({ queryKey: ["g-an-missions", ...rangeKeyArr],       queryFn: () => getMissionAnalytics(range) });
  const achievementsQ = useQuery({ queryKey: ["g-an-ach", ...rangeKeyArr],            queryFn: () => getAchievementAnalytics(range) });
  const leagueQ       = useQuery({ queryKey: ["g-an-league"],                          queryFn: getLeagueDistribution, staleTime: 60_000 });
  const riskQ         = useQuery({ queryKey: ["g-an-risk", ...rangeKeyArr],           queryFn: () => getRiskEmployees(range) });
  ...
  const colors = ["hsl(var(--primary))", "hsl(var(--accent-foreground))", "hsl(var(--destructive))", "hsl(var(--muted-foreground))", "hsl(var(--secondary-foreground))", "hsl(var(--ring))"];
  ...
  return (
    <div className="container py-6 space-y-6" dir="rtl">
        ...
        <CardContent className="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <div className="space-y-1">
            ...
            <Select value={employeeId} onValueChange={setEmployeeId}>
              <SelectTrigger><SelectValue placeholder="همه کارمندان" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="__all">همه کارمندان</SelectItem>
                {(employeesQ.data ?? []).map((e) => (
                  <SelectItem key={e.id} value={e.id}>{e.full_name ?? "بدون نام"}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
          ...
          <div className="space-y-1">
            ...
            <Select value={eventType} onValueChange={setEventType}>
              ...
              <SelectContent>
                ...
                {EVENT_TYPES.map((t) => (
                  <SelectItem key={t} value={t}>{EVENT_TYPE_FA[t]}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
      ...
      {seasonFallbackNotice ? (
        <Alert>
          <AlertDescription>فصل فعالی پیدا نشد؛ بازه ۳۰ روز اخیر نمایش داده می‌شود.</AlertDescription>
        </Alert>
      ) : null}
      <Alert variant="default">
        <AlertDescription>
          منبع XP تجمیعی قابل اتکا نیست؛ به همین دلیل کارت «مجموع XP» در این داشبورد نمایش داده نمی‌شود.
        </AlertDescription>
      </Alert>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
        {summaryQ.isLoading ? (
          ...
          <Card className="col-span-full"><CardContent className="p-4 text-sm text-destructive">{(summaryQ.error as Error).message}</CardContent></Card>
        ) : (
          <>
            <StatCard label="مجموع رویدادها" value={summaryQ.data!.total_events.toLocaleString("fa-IR")} icon={Activity} />
            <StatCard label="مدال‌های آزادشده" value={summaryQ.data!.total_achievements.toLocaleString("fa-IR")} icon={Trophy} />
            <StatCard label="مأموریت‌های تکمیل‌شده" value={summaryQ.data!.total_missions_completed.toLocaleString("fa-IR")} icon={Target} />
            <StatCard label="کارمندان فعال" value={summaryQ.data!.active_employees.toLocaleString("fa-IR")} icon={Users} />
            <StatCard label="میانگین رویداد به ازای کارمند" value={Number(summaryQ.data!.avg_events_per_employee).toLocaleString("fa-IR")} icon={Activity} />
          </>
        )}
      </div>
      ...
      {/* Activity trend */}
      <Card>
        <CardHeader><CardTitle>روند فعالیت‌ها</CardTitle></CardHeader>
        <CardContent>
          {trendQ.isLoading ? <Skeleton className="h-64 w-full" /> :
            trendChartData.data.length === 0 ? (
              <div className="text-sm text-muted-foreground py-8 text-center">داده‌ای در این بازه پیدا نشد.</div>
            ) : (
      ...
      {/* Top Employees */}
      <Card>
        <CardHeader><CardTitle>کارمندان برتر</CardTitle></CardHeader>
        <CardContent>
          {topQ.isLoading ? <div className="flex justify-center py-6"><Loader2 className="animate-spin h-5 w-5" /></div> :
           (topQ.data?.length ?? 0) === 0 ? <div className="text-sm text-muted-foreground py-6 text-center">داده‌ای در این بازه پیدا نشد.</div> : (
                ...
                <TableBody>
                  {topQ.data!.map((r) => (
                    <TableRow key={r.employee_id}>
                      ...
                      <TableCell>{r.current_league ? (TIER_FA[r.current_league as LeagueTier] ?? r.current_league) : "—"}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
      ...
      {/* KPI Effectiveness */}
      <Card>
        <CardHeader><CardTitle>اثربخشی قوانین KPI</CardTitle></CardHeader>
        <CardContent>
          {kpiEffQ.isLoading ? <Skeleton className="h-32 w-full" /> :
            (kpiEffQ.data?.length ?? 0) === 0 ? <div className="text-sm text-muted-foreground py-6 text-center">داده‌ای در این بازه پیدا نشد.</div> : (
            <div className="overflow-x-auto">
              <Table>
                <TableHeader><TableRow>
                ...
                <TableBody>
                  {kpiEffQ.data!.map((r) => (
                    <TableRow key={r.event_key}>
                      ...
                      <TableCell>{r.title_fa ?? <span className="text-xs text-muted-foreground">— بدون قانون —</span>}</TableCell>
                      <TableCell>{r.events_count.toLocaleString("fa-IR")}</TableCell>
                      <TableCell>{r.xp_amount != null ? Number(r.xp_amount).toLocaleString("fa-IR") : "—"}</TableCell>
                      <TableCell>{r.title_fa == null ? <Badge variant="outline">بدون قانون</Badge> : r.is_active ? <Badge>فعال</Badge> : <Badge variant="secondary">غیرفعال</Badge>}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
      ...
      {/* Mission Analytics */}
      <Card>
        <CardHeader><CardTitle>تحلیل مأموریت‌ها</CardTitle></CardHeader>
        <CardContent>
          {missionsQ.isLoading ? <Skeleton className="h-32 w-full" /> :
            (missionsQ.data?.length ?? 0) === 0 ? <div className="text-sm text-muted-foreground py-6 text-center">داده‌ای در این بازه پیدا نشد.</div> : (
            <div className="overflow-x-auto">
              <Table>
                <TableHeader><TableRow>
                ...
                <TableBody>
                  {missionsQ.data!.map((r) => (
                    <TableRow key={r.mission_id}>
                      <TableCell>{r.title_fa} {!r.enabled ? <Badge variant="secondary" className="ml-1">غیرفعال</Badge> : null}</TableCell>
      ...
      <Card>
        <CardHeader><CardTitle>تحلیل مدال‌ها</CardTitle></CardHeader>
        <CardContent>
          {achievementsQ.isLoading ? <Skeleton className="h-32 w-full" /> :
            (achievementsQ.data?.length ?? 0) === 0 ? <div className="text-sm text-muted-foreground py-6 text-center">داده‌ای در این بازه پیدا نشد.</div> : (
            <div className="overflow-x-auto">
              <Table>
                <TableHeader><TableRow>
                  <TableHead>عنوان</TableHead>
                ...
                <TableBody>
                  {achievementsQ.data!.map((r) => (
                    <TableRow key={r.achievement_id}>
                      ...
                      <TableCell>{r.xp_reward.toLocaleString("fa-IR")}</TableCell>
                      <TableCell>{r.enabled ? <Badge>فعال</Badge> : <Badge variant="secondary">غیرفعال</Badge>}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
      ...
      {/* League Distribution */}
      <Card>
        <CardHeader><CardTitle>توزیع لیگ‌ها</CardTitle></CardHeader>
        <CardContent>
          {leagueQ.isLoading ? <Skeleton className="h-48 w-full" /> :
            (leagueQ.data?.length ?? 0) === 0 ? <div className="text-sm text-muted-foreground py-6 text-center">داده‌ای در دسترس نیست.</div> : (
            <div className="h-56 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={(leagueQ.data ?? []).map((r) => ({ name: TIER_FA[r.league as LeagueTier] ?? r.league, value: Number(r.employees_count) }))}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="name" />
                  <YAxis allowDecimals={false} />
                  <Tooltip />
      ...
      <Card>
        ...
        <CardContent>
          {riskQ.isLoading ? <Skeleton className="h-32 w-full" /> :
            (riskQ.data?.length ?? 0) === 0 ? <div className="text-sm text-muted-foreground py-6 text-center">همه فروشندگان فعالیت کافی دارند.</div> : (
            <div className="overflow-x-auto">
              <Table>
                <TableHeader><TableRow>
                  <TableHead>کارمند</TableHead>
