...
type Indicator = {
  id: string; code: string; title_fa: string; category: string; unit: string; is_active: boolean; sort_order: number;
};
type Source = { id: string; code: string; title_fa: string; source_type: string; is_enabled: boolean };
type Tick = {
  id: string; indicator_id: string; source_id: string; value: number; unit: string;
  observed_at: string; change_amount: number | null; change_percent: number | null;
  status: string; note: string | null; created_at: string;
};

const STATUS_LABEL: Record<string, string> = { accepted: "تأییدشده", suspect: "مشکوک", rejected: "ردشده" };
const STATUS_VARIANT: Record<string, "default" | "secondary" | "destructive"> = {
  accepted: "default", suspect: "secondary", rejected: "destructive",
};
const CATEGORY_LABEL: Record<string, string> = {
  currency: "ارز", gold: "طلا", coin: "سکه", official: "رسمی", crypto: "رمزارز", manual: "دستی",
};
...
function MarketRatesWorkshopPage() {
  ...
  const indicatorsQ = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("market_indicators").select("*").eq("is_active", true)
  ...
  const sourcesQ = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("market_rate_sources").select("*").eq("is_enabled", true)
        .order("title_fa", { ascending: true }).limit(50);
      if (error) throw error;
  ...
  const latestQ = useQuery({
    ...
    queryFn: async () => {
      ...
      await Promise.all(indicatorsQ.data.map(async (ind) => {
        if (isPrivileged) {
          const { data } = await supabase
            ...
            .eq("indicator_id", ind.id).eq("status", "accepted")
            .order("observed_at", { ascending: false }).limit(1).maybeSingle();
          if (data) out[ind.id] = data as unknown as Tick;
        } else {
          const { data } = await supabase
            .rpc("list_market_rate_ticks_public", { p_indicator_id: ind.id, p_limit: 1 });
          const row = (data ?? [])[0];
  ...
  const indicatorMap = useMemo(() => {
    ...
    (indicatorsQ.data ?? []).forEach(i => { m[i.id] = i; });
    return m;
  }, [indicatorsQ.data]);
  const sourceMap = useMemo(() => {
    const m: Record<string, Source> = {};
    (sourcesQ.data ?? []).forEach(s => { m[s.id] = s; });
    return m;
  }, [sourcesQ.data]);
    ...
    <div dir="rtl" className="space-y-5">
      ...
      <section>
        ...
        {indicatorsQ.isLoading ? (
          ...
          <div className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">شاخصی تعریف نشده است.</div>
        ) : (
                ...
                <Card key={ind.id}>
                  <CardHeader className="pb-2">
                    <CardTitle className="flex items-center justify-between text-base">
                      ...
                      <Badge variant="outline" className="text-xs">{CATEGORY_LABEL[ind.category] ?? ind.category}</Badge>
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                    {!t ? (
                      ...
                      <div className="space-y-1">
                        <div className="text-2xl font-bold">{formatNumber(Number(t.value))} <span className="text-xs font-normal text-muted-foreground">{t.unit === "toman" ? "تومان" : t.unit}</span></div>
                        {t.change_amount !== null && (
                          <div className={`flex items-center gap-1 text-xs ${isUp ? "text-emerald-600" : isDown ? "text-rose-600" : "text-muted-foreground"}`}>
                            {isUp && <TrendingUp className="h-3 w-3" />}
                            {isDown && <TrendingDown className="h-3 w-3" />}
                            <span>{formatNumber(Math.abs(Number(t.change_amount)))}</span>
                            {t.change_percent !== null && <span>({Number(t.change_percent).toFixed(2)}٪)</span>}
                          </div>
                        )}
                        ...
                        <Badge variant={STATUS_VARIANT[t.status] ?? "default"} className="text-xs">{STATUS_LABEL[t.status] ?? t.status}</Badge>
                      </div>
                    )}
                  </CardContent>
                </Card>
        ...
        <CardContent className="space-y-3">
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
            <Select value={filterIndicator} onValueChange={setFilterIndicator}>
              <SelectTrigger><SelectValue placeholder="شاخص" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="all">همه شاخص‌ها</SelectItem>
                {indicatorsQ.data?.map(i => <SelectItem key={i.id} value={i.id}>{i.title_fa}</SelectItem>)}
              </SelectContent>
            </Select>
            {isPrivileged && (
              <>
                <Select value={filterSource} onValueChange={setFilterSource}>
                  <SelectTrigger><SelectValue placeholder="منبع" /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="all">همه منابع</SelectItem>
                    {sourcesQ.data?.map(s => <SelectItem key={s.id} value={s.id}>{s.title_fa}</SelectItem>)}
                  </SelectContent>
                </Select>
          ...
          {historyQ.isLoading ? (
            ...
            <div className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">رکوردی یافت نشد.</div>
          ) : (
                ...
                <tbody>
                  {historyQ.data.map((t) => (
                    <tr key={t.id} className="border-b last:border-0">
                      ...
                      <td className="px-2 py-2 font-semibold">{formatNumber(Number(t.value))} <span className="text-xs text-muted-foreground">{t.unit === "toman" ? "تومان" : t.unit}</span></td>
                      <td className="px-2 py-2 text-xs">
                        {t.change_amount !== null ? (
                          <span className={Number(t.change_amount) > 0 ? "text-emerald-600" : Number(t.change_amount) < 0 ? "text-rose-600" : ""}>
                            {Number(t.change_amount) > 0 ? "+" : ""}{formatNumber(Number(t.change_amount))}
                            {t.change_percent !== null && ` (${Number(t.change_percent).toFixed(2)}٪)`}
                          </span>
                        ) : "—"}
                      </td>
                      <td className="px-2 py-2">{sourceMap[t.source_id]?.title_fa ?? "—"}</td>
                      <td className="px-2 py-2 text-xs text-muted-foreground">{formatDateFa(t.observed_at)}</td>
                      <td className="px-2 py-2"><Badge variant={STATUS_VARIANT[t.status] ?? "default"} className="text-xs">{STATUS_LABEL[t.status] ?? t.status}</Badge></td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
function NewTickDialog({ indicators, sources, onDone }: {
  indicators: Indicator[]; sources: Source[]; onDone: () => void;
}) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button size="sm"><Plus className="ml-1 h-4 w-4" /> ثبت نرخ جدید</Button>
      </DialogTrigger>
      <DialogContent dir="rtl" className="sm:max-w-md">
        <DialogHeader><DialogTitle>ثبت دستی نرخ</DialogTitle></DialogHeader>
        <div className="space-y-3">
          <div className="space-y-1">
            <Label>شاخص</Label>
            <Select value={indicatorId} onValueChange={setIndicatorId}>
              <SelectTrigger><SelectValue placeholder="انتخاب شاخص" /></SelectTrigger>
              <SelectContent>
                {indicators.map(i => <SelectItem key={i.id} value={i.id}>{i.title_fa}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-1">
            ...
            <Select value={sourceId} onValueChange={setSourceId}>
              ...
              <SelectContent>
                {sources.map(s => <SelectItem key={s.id} value={s.id}>{s.title_fa}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-1">
            <Label>مقدار</Label>
            <Input type="number" inputMode="decimal" min="0" step="any" value={value} onChange={(e) => setValue(e.target.value)} placeholder="مثلاً ۸۵۰۰۰" />
          </div>
          <div className="space-y-1">
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => setOpen(false)} disabled={mut.isPending}>انصراف</Button>
          <Button onClick={() => mut.mutate()} disabled={mut.isPending}>{mut.isPending ? "در حال ثبت…" : "ثبت نرخ"}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
