      ...
      GET: async ({ request, params }) => {
        ...
        if (!SLUG_RE.test(slug)) {
          ...
          return jsonResponse(400, {
            error: "invalid_slug",
            message: "شناسه slug جدول نامعتبر است.",
          });
        }
        ...
        if (tErr) {
          ...
          return jsonResponse(500, {
            error: "server_error",
            message: "خطا در خواندن اطلاعات جدول.",
          });
        }
        if (!table) {
          logBotUsage({
          ...
          return jsonResponse(404, {
            ...
            message: "جدولی با این slug یافت نشد.",
          });
        }
        ...
        const { data: cols } = await supabaseAdmin
          ...
          .select(
            "column_key, label, data_type, is_required, is_filterable, is_editable_by_bot, is_computed, formula_key, sort_order",
          )
          .eq("table_id", table.id)
