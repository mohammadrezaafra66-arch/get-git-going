              ...
              <Card key={t.id as string} className="hover:border-primary/50 transition-colors">
                <CardContent className="p-4 space-y-3">
                        ...
                        return (
                          <>
                            ...
                            {lvl === "custom" && allowed.length > 0 && (
                              <span
                                className="text-[10px] text-muted-foreground text-left max-w-[160px] truncate"
                                title={allowed.join("، ")}
                              >
                                {allowed
                                  .map(
                                    (r) => SELECTABLE_ROLES.find((x) => x.value === r)?.label ?? r,
                                  )
                                  .join("، ")}
                              </span>
                            )}
                  ...
                  <Button asChild variant="outline" size="sm" className="w-full">
                    <Link to="/data-tables/$tableId" params={{ tableId: t.id as string }}>
                      مشاهده و مدیریت
                    </Link>
                  </Button>
                </CardContent>
