...
function PayablesPage() {
  ...
  const listQ = useQuery({
    queryKey: ["payables-list", fromIso, toIso, dueFilter, debouncedSearch, pageSize, page, includePaid],
    queryFn: async () => {
  ...
  return (
    <div className="space-y-4 p-4" dir="rtl">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        <SummaryCard label="کل بدهی‌ها" value={summaryQ.isLoading ? "…" : fmtMoney(summary?.total_outstanding ?? 0)} tone="info" />
        ...
        <CardContent className="p-4 space-y-3">
          ...
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center gap-2">
              <Switch id="include-paid" checked={includePaid} onCheckedChange={(v) => { setIncludePaid(!!v); setPage(0); }} />
              <Label htmlFor="include-paid" className="cursor-pointer">نمایش پرداخت‌شده‌ها</Label>
            </div>
            {(fromDate || toDate || search || dueFilter !== "all" || includePaid) && (
              <Button variant="ghost" size="sm" onClick={() => { setFromDate(undefined); setToDate(undefined); setSearch(""); setDueFilter("all"); setIncludePaid(false); setPage(0); }}>
      ...
      <Card>
        <CardContent className="p-0">
          {listQ.isLoading ? (
            ...
            <div className="p-10 text-center text-muted-foreground">بدهی‌ای برای نمایش وجود ندارد.</div>
          ) : (
            <>
              {/* Desktop table */}
                  ...
                  <TableBody>
                    {listQ.data!.map((r) => (
                      <TableRow key={r.purchase_id}>
                        ...
                        <TableCell>{r.payment_term_days != null ? `${toFaDigits(String(r.payment_term_days))} روز` : NA}</TableCell>
                        <TableCell>{fmtMoney(r.purchase_total_amount, r.currency)}</TableCell>
                        <TableCell>{fmtMoney(r.cash_price, r.currency)}</TableCell>
                        <TableCell className="font-semibold">{fmtMoney(r.outstanding_amount, r.currency)}</TableCell>
                        <TableCell>
                          {r.is_paid ? (
                        ...
                        <TableCell>
                          <Button size="sm" variant="outline" onClick={() => setDetailPurchaseId(r.purchase_id)}>
                ...
                {listQ.data!.map((r) => (
                  <div key={r.purchase_id} className="p-3 space-y-2">
                    <div className="flex justify-between items-start">
                      ...
                      {r.is_paid ? <Badge variant="secondary">پرداخت‌شده</Badge>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      ...
                      <div><span className="text-muted-foreground">مانده: </span><span className="font-semibold">{fmtMoney(r.outstanding_amount, r.currency)}</span></div>
                      <div><span className="text-muted-foreground">مبلغ: </span>{fmtMoney(r.purchase_total_amount, r.currency)}</div>
                      <div><span className="text-muted-foreground">تاریخ خرید: </span>{fmtDate(r.purchase_date)}</div>
                    </div>
                    <Button size="sm" variant="outline" className="w-full" onClick={() => setDetailPurchaseId(r.purchase_id)}>
                      <Eye className="h-4 w-4 ml-1" /> جزئیات
                    </Button>
                  </div>
                ))}
      ...
      {/* Detail Sheet */}
      <Sheet open={!!detailPurchaseId} onOpenChange={(o) => { if (!o) setDetailPurchaseId(null); }}>
              ...
              return (
                <>
                  <Card><CardContent className="p-3 space-y-1 text-sm">
                    <div><span className="text-muted-foreground">تأمین‌کننده: </span>{head.supplier_name || NA}</div>
                    <div><span className="text-muted-foreground">شناسه خرید: </span><span className="font-mono">{shortId(head.purchase_id)}</span></div>
                    <div><span className="text-muted-foreground">تاریخ خرید: </span>{fmtDate(head.purchase_date)}</div>
                    <div><span className="text-muted-foreground">سررسید: </span>{fmtDate(head.due_date)}</div>
                    <div><span className="text-muted-foreground">مدت پرداخت: </span>{head.payment_term_days != null ? `${toFaDigits(String(head.payment_term_days))} روز` : NA}</div>
                    {head.paid_at && <div><span className="text-muted-foreground">پرداخت‌شده در: </span>{fmtDate(head.paid_at)}</div>}
                  </CardContent></Card>
                  <Card><CardContent className="p-3 space-y-1 text-sm">
                    <div><span className="text-muted-foreground">مبلغ خرید: </span>{fmtMoney(head.purchase_total_amount, head.currency)}</div>
                    <div><span className="text-muted-foreground">قیمت نقدی: </span>{fmtMoney(head.cash_price, head.currency)}</div>
                    <div><span className="text-muted-foreground">ارز: </span>{head.currency || NA}</div>
                    <div className="font-semibold text-base"><span className="text-muted-foreground font-normal">مانده: </span>{fmtMoney(head.outstanding_amount, head.currency)}</div>
                  ...
                  <div>
                    ...
                    {items.length === 0 ? (
                      <div className="text-sm text-muted-foreground">اقلامی برای این خرید ثبت نشده است.</div>
                    ) : (
                          ...
                          <Card key={it.item_id}>
                            <CardContent className="p-3 space-y-1 text-sm">
                              ...
                              <div className="grid grid-cols-2 gap-2">
                                <div><span className="text-muted-foreground">تعداد: </span>{it.item_quantity != null ? toFaDigits(String(it.item_quantity)) : NA}</div>
                                <div><span className="text-muted-foreground">قیمت واحد: </span>{fmtMoney(it.item_unit_price, head.currency)}</div>
                                <div className="col-span-2"><span className="text-muted-foreground">جمع ردیف: </span><span className="font-semibold">{fmtMoney(it.item_line_total, head.currency)}</span></div>
                              </div>
                            </CardContent>
                          </Card>
                        ))}
