...
notification_queue        presence_logs (via is_user_online)
```

> **دو مورد آخر با تحلیل ایستا پیدا نشدند، با شواهد زمان اجرا پیدا شدند.**
> بازبینی مستقل `pg_stat_statements` را پرسید که نقش `anon` واقعاً چه چیزی
> **اجرا کرده** — روشی که به شکل کد وابسته نیست و با `fetch` دستی یا import
> پویا هم دور نمی‌خورد. نتیجه: `notification_queue` پانزده بار به‌عنوان
> `anon` کوئری شده. مسیرش عمومی نیست — `NotificationBell.tsx` و
> `useIsOnline.ts` زیر `AppHeader` یعنی پوستهٔ احرازشده — ولی کامپوننت
> **پیش از چسبیدن توکن نشست** شلیک می‌کند، پس درخواست به‌عنوان `anon` می‌رود.
>
> اهمیتش برای OG-30: این دو در فهرست نام‌بردهٔ بالا نبودند و فقط در پیوست
> انبوه ۲۰۲ نامی ظاهر می‌شدند. یک REVOKE دسته‌جمعی که از آن فهرست ساخته شود،
> **اولین رندر زنگ اعلان را در هر بار بارگذاری صفحه ۴۰۱ می‌کند.**

هر هفت‌تا به‌عنوان `anon` صفر ردیف می‌دهند. این‌ها **دادهٔ منتشرشده نیستند، راه‌اندازی
