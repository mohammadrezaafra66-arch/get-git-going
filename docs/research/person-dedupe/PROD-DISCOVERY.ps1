...
# Run in Admin or normal PowerShell on the PRODUCTION laptop (192.168.170.10).
...
function KV([string]$k, $v) { Write-Host ("{0,-28} {1}" -f $k, $v) }
...
foreach ($p in $candidates) {
  $exists = Test-Path $p
  $git = Test-Path (Join-Path $p ".git")
  $compose = Test-Path (Join-Path $p "deploy\lan\docker-compose.yml")
  $composeAlt = Test-Path (Join-Path $p "docker-compose.yml")
...
$web = docker ps -a --filter "name=$webName" --format "{{.Names}}|{{.Status}}|{{.Ports}}" 2>$null
KV "docker_ps_web" $web

if ($web) {
  KV "compose_project" (docker inspect $webName --format '{{index .Config.Labels "com.docker.compose.project"}}' 2>$null)
  KV "compose_working_dir" (docker inspect $webName --format '{{index .Config.Labels "com.docker.compose.project.working_dir"}}' 2>$null)
  KV "compose_config_files" (docker inspect $webName --format '{{index .Config.Labels "com.docker.compose.project.config_files"}}' 2>$null)
  KV "image" (docker inspect $webName --format '{{.Config.Image}}' 2>$null)
  KV "started" (docker inspect $webName --format '{{.State.StartedAt}}' 2>$null)
  Write-Host "--- ports / publish ---"
  docker port $webName 2>$null
  Write-Host "--- APP_/VITE_/env stamp ---"
  docker inspect $webName --format "{{range .Config.Env}}{{println .}}{{end}}" 2>$null |
    Select-String -Pattern "APP_GIT_SHA|GIT_SHA|BUILD_TIME|APP_PORT|SITE_URL|SUPABASE|POSTGRES_DB|ISSABEL|PRICING|OLLAMA|MARKETING"
}
...
$gitRoots = @()
foreach ($p in $candidates) {
  if (Test-Path (Join-Path $p ".git")) { $gitRoots += $p }
  elseif ((Test-Path (Join-Path $p "app\.git"))) { $gitRoots += (Join-Path $p "app") }
}
...
function Show-EnvKeyNames([string]$envPath) {
  if (-not (Test-Path $envPath)) { KV $envPath "MISSING"; return }
  KV $envPath "present"
  ...
  $want = @("APP_PORT","SITE_URL","POSTGRES_DB","POSTGRES_PASSWORD","SUPABASE_URL","ISSABEL_CDR_HOST","ISSABEL_CDR_USER","ISSABEL_CDR_PASSWORD","ISSABEL_CDR_DB","ISSABEL_IMPORT_WORKER_TOKEN","ISSABEL_AMI_HOST","ISSABEL_AMI_USER","ISSABEL_AMI_SECRET","PRICING_WORKER_TOKEN","GIT_SHA")
  foreach ($k in $want) {
    $hit = $keys -contains $k
    # presence only — never print values
    $nonEmpty = $false
    if ($hit) {
      $line = Get-Content $envPath | Where-Object { $_ -match ("^\s*" + [regex]::Escape($k) + "\s*=") } | Select-Object -First 1
      if ($line) {
        $val = ($line -split "=", 2)[1]
...
docker ps -a --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}" 2>$null | Select-String -Pattern "db|postgres|afrakala|NAMES"
...
foreach ($d in $dbCandidates) {
  $exists = docker ps -a --format "{{.Names}}" 2>$null | Where-Object { $_ -eq $d }
  if ($exists) {
    KV "db_container" $d
    docker inspect $d --format "{{range .Config.Env}}{{println .}}{{end}}" 2>$null |
      Select-String -Pattern "POSTGRES_DB|POSTGRES_USER|POSTGRES_HOST"
  }
}

Section "7) MIGRATION LEDGER (read-only) — last versions + our cutover versions"
# Tries supabase_admin with password from .env.lan of live tree if available.
function Try-Ledger([string]$dbContainer, [string]$dbName, [string]$envFile) {
  if (-not (Test-Path $envFile)) { return $false }
  $pwLine = Get-Content $envFile | Where-Object { $_ -match '^\s*POSTGRES_PASSWORD=' } | Select-Object -First 1
  if (-not $pwLine) { Write-Host "No POSTGRES_PASSWORD in $envFile"; return $false }
  $pw = ($pwLine -split "=", 2)[1]
  Write-Host "Trying ledger: container=$dbContainer db=$dbName env=$envFile"
  $sql = @"
...
if (-not $tried) { Write-Host "Could not query ledger automatically — paste docker/db names manually if needed." }
...
function Grep-Compose([string]$compose) {
  if (-not (Test-Path $compose)) { KV $compose "MISSING"; return }
  KV $compose "present"
