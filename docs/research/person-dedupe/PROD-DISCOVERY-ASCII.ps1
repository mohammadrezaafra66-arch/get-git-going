# =============================================================================
# AfraKala PROD-DISCOVERY - read-only, PowerShell 5.1, ASCII-ONLY
# Run on PRODUCTION. Does NOT change anything. Paste full output back.
# =============================================================================
...
$webLine = (Run-Cmd "docker" @("ps", "-a", "--filter", "name=$webName", "--format", $fmt) | Select-Object -First 1)
...
Section "5 .env.lan KEY NAMES ONLY"
...
$tableFmt = 'table {{.Names}}\t{{.Status}}\t{{.Ports}}'
Run-Cmd "docker" @("ps", "-a", "--format", $tableFmt)
Write-Host "--- POSTGRES_DB / POSTGRES_USER ---"
foreach ($d in @("afrakala-lan-db", "afrakala-db", "supabase-db", "db")) {
  $nameFmt = '{{.Names}}'
  $names = Run-Cmd "docker" @("ps", "-a", "--format", $nameFmt)
  if ($names -contains $d) {
    KV "db_container" $d
...
Write-Host "Q4 FEATURES              = full-main"
Write-Host "Q5 MIGRATE_PROD_APPROVED = YES I approve"
Write-Host "Q6 ISSABEL_LATER_OK      = yes leave Issabel for later"
