...
-- Reverse of 545.
-- Order: restore person_merge WITHOUT sales_interactions.person_id FIRST,
-- then DROP TABLE (migration 328 registry gate).
...
DROP TABLE IF EXISTS public.sales_interactions;

DO $do$
BEGIN
  IF to_regclass('public.sales_interactions') IS NOT NULL THEN
    RAISE EXCEPTION '545-down: table still present';
  END IF;
  IF EXISTS (
    SELECT 1 FROM public.person_merge_registry_keys()
     WHERE registry_key = 'sales_interactions.person_id'
  ) THEN
    RAISE EXCEPTION '545-down: registry still lists sales_interactions.person_id';
  END IF;
  PERFORM public.assert_person_fk_registry();
  RAISE NOTICE '545-down OK';
END
$do$;
