...
| 1.9 | **۵۳۳/۵۳۴ به release امشب نمی‌روند — OG-L، و راه‌حلش یک GUC نشستی است** | agent فردا | 🔴 **گیت release بعدی.** شاخهٔ نصب `http` در ۵۳۳ روی `current_database() = 'postgres'` گارد شده، پس در **هر** rehearsal غلط است و فقط روی production درست — تنها جایی که هرگز اجرا نشده. یعنی `Expect`های بلوک ۱۴ از اجرایی استخراج شده بودند که آن شاخه در آن اصلاً fire نکرد، و نام `prod_rehearsal_e4b` را می‌بردند. **انتظاری که روی مسیرِ طی‌نشده اندازه‌گیری شده، انتظار نیست.** **راه‌حل:** گارد را روی یک **GUC نشستی** ببند — مثلاً `app.release_target` — که `release/rehearse.ps1` روی دیتابیس scratch و `release/apply-release.ps1` روی production ست می‌کنند. آن‌وقت شاخهٔ نصب **در rehearsal هم اجرا می‌شود**، `Expect`ها روی همان مسیری اندازه‌گیری می‌شوند که production طی می‌کند، و گارد هنوز هرجا کسی GUC را ست نکرده fire نمی‌کند. سپس **مینی‌گیت خودش**. حذفشان ۵۳۷ را هم به **۲۱۴** برمی‌گرداند (بدون `cron_run_log` یک جدول کمتر) — عددی که rehearsal دوباره مشتق می‌کند، نه دست |
