...
| 2.7 | **صفحهٔ عمومی لیست فروش برای بازدیدکنندهٔ ناشناس باز نمی‌شود — طبق طراحی (D-24)** | `anon` هیچ `SELECT`ی روی `sale_lists` و `sale_list_items` ندارد، پس loader در **اولین** کوئری‌اش می‌افتد: `ERROR: permission denied for table sale_lists` — اندازه‌گیری رفتاری روی shape production **بدون هیچ‌کدام از دوازده مهاجرت**. این نتیجهٔ ۴۷۷/۵۲۳/۵۲۴ است که دسترسی جدولی `anon` را به یازده جدول محدود کردند و این دو جزوشان نیستند. کسی متوجه نشد چون هر **۱۹ لیست فروش draft‌اند**. **این سؤال باز نیست — تصمیمش گرفته شده: D-24 (۱۵ شهریور ۱۴۰۵): «لیست فروش عمومی برای بازدیدکنندهٔ خارج‌شده بسته می‌ماند؛ فقط ۴۰۴ باید با پیام درست جایگزین شود.»** پس یافتهٔ devil's-advocate تأیید مستقل اجرای D-24 است، نه یک شکاف. **کار باقی‌مانده همان کوچکی است که خود D-24 نام برد:** جایگزینی ۴۰۴ خالی با پیام فارسی که بگوید این لیست عمومی نیست. **هیچ grantی به `anon` داده نمی‌شود.** خارج از این release | convergence موج C + D-24 |
...
| 7.8 | **`viewer` روی هفت شیء definer — با گاردِ درون‌بدنه، نه با grant** | **تصمیم گرفته شد (C-4):** مکانیزم همان گارد داخل بدنهٔ تابع می‌ماند. بستن در سطح `grant` تلهٔ ۳۹۵ است: هر هفت شیء **۱۳ محل فراخوانی زنده در `src/`** دارند و `og61:134` صراحتاً می‌خواهد این‌ها برای `authenticated` باز بمانند. کار باقی‌مانده بررسی خود گاردهاست در امنیت ۳، نه یک مهاجرت |
| 7.9 | **`http` بدون allowlist میزبان** | **تصمیم گرفته شد:** فعلاً `REVOKE EXECUTE` روی هر ۱۹ تابع افزونه (که مهاجرت ۵۳۳ از `pg_depend` مشتق می‌کند) **همان allowlist است**. محدودسازی مقصدِ شبکه یک آیتم سیاست شبکه است، نه مهاجرت. امنیت ۳ |
| 7.10 | **OG-K — `anon` روی production ۱۹ ستون از `products` را می‌خواند که مهاجرت ۳۸۸ آن‌ها را بسته بود** | دامپ production خودش `GRANT SELECT ON TABLE public.products TO anon` دارد (و همین برای `categories` و `academy_quiz_questions`). مهاجرت‌های ۳۸۸ و ۳۹۰ دقیقاً برای حذف همین grantها نوشته شده‌اند و **هر دو روی production ردیف لجر دارند** — دو ردیف لجر دیگر که دروغ می‌گویند. ۳۸۸ فقط ۹ ستون را برای `anon` باز می‌گذارد؛ production هر ۲۸ ستون را می‌دهد. اندازه‌گیری رفتاری با نشست واقعی `anon`: **۳۸۱ ردیف** خواندنی، شامل `sku, created_by, updated_by, accounting_code, dedup_key, barcode, technical_notes, torob_url`. RLS روشن است با ۷ policy و **ستون‌ها را فیلتر نمی‌کند**، پس کاهش‌دهنده نیست. **تصمیم مالک (۲۲ شهریور): با آگاهی منتقل می‌شود؛ ۵۴۰ امشب نوشته نمی‌شود** — در دامنهٔ این release نبود، هیچ گیتی ندیده‌اش، و افشای فقط‌خواندنیِ ستون‌های کالاست نه اعتبار، پول یا PII. یک مهاجرت اثبات‌نشده بعد از بسته شدن گیت دقیقاً همان چیزی است که قواعد falsification دربارهٔ آن هشدار می‌دهند. **۵۴۰ گیت خودش را می‌گیرد** — امنیت ۳ یا یک pass کوچک اختصاصی، کاتالوگ‌محور، **هرگز فهرست ستونی ثابت** |
