...
DO $do$
DECLARE
  aid uuid; sid uuid; pid uuid; oid uuid; nq1 int; ntitle text;
BEGIN
  ...
  IF aid IS NULL OR sid IS NULL OR pid IS NULL THEN RETURN; END IF;
  BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    INSERT INTO _c4 VALUES ('insert_fail', SQLERRM); RETURN;
  END;
  BEGIN
  ...
  END;
  SELECT count(*), max(nq.title) INTO nq1, ntitle
  FROM notification_queue nq
  WHERE nq.user_id=sid AND nq.type='sales_interaction_assigned' AND nq.reference_id=oid;
  INSERT INTO _c4 VALUES (
    'notify',
    'count='||nq1||' title='||coalesce(ntitle,'null')||
    ' hex='||coalesce(encode(convert_to(coalesce(ntitle,''),'UTF8'),'hex'),'')
  );
END
$do$;
SELECT * FROM _c4;
...
execFileSync("docker", ["exec", "-i", "afrakala-lan-db", "sh", "-c", "cat > /tmp/c4b.sql"], {
