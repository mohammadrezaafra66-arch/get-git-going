...
- **دستیار** → `/pricing/market-intelligence`, `/pricing/product-recommendations`, `/pricing/price-alerts`, `/marketing/suggestions`, `/marketing/suggestions-history`, `/messages`, `/knowledge`, `/academy`
...
- **تحلیل** → گزارش‌ها، لاگ اشتراک، گیمیفیکیشن، لاگ فعالیت، جداول داده پویا
- **مدیریت** → کاربران، نقش‌ها، تنظیمات، ارز، تسویه، API ربات، ابزارها، بازخورد

**بدون ماژول fallback.** دقیقاً ۷ ماژول اصلی وجود دارد. تمام آیتم‌های کاربری `NAV_ITEMS` به‌صورت دستی به یکی از ۷ ماژول map شده‌اند (بازخورد→مدیریت، دانش/آکادمی→دستیار، جداول داده→تحلیل). روت‌های کاملاً فنی/داخلی همچنان از طریق ناوبری مستقیم URL و workflowهای موجود قابل دسترس‌اند اما در منوی fallback ظاهر نمی‌شوند.
