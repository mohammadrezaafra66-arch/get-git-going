...
Status:               کامل. ۳۸۶+۳۸۷ اعمال. بازبینی دور ۱ پاسخ داده شد. R9 = BLOCKED.
...
Deployed APP_GIT_SHA: e66e3759 در شروع → a19fd811 اکنون. **مالک در ۱۵:۳۸ بازسازی کرد،
                      نه من.** diff در src/ خالی است، پس کد اجراشونده همان است.
...
## R9 — **BLOCKED. قابل اندازه‌گیری نیست، و به‌عنوان نتیجه گزارش نمی‌شود.**

دو بار اجرا شد و **هر دو بار محیط زیرش عوض شد.** هیچ‌کدام اندازه‌گیری معتبر نیست.

### اجرای یکم — باطل، و تقصیرش با من

```
شروع ≈ 13:56، مدت 1.6h
14:21:44  afrakala-lan-rest restart      ← من، پس از اعمال مهاجرت ۳۸۷
14:24:17  afrakala-lan-caddy started     ← کار موازی HTTPS مالک
نتیجه: 524 passed / 52 failed  (خط پایهٔ OG-43: ۴۱)
```

یازده شکست تازه، همه با الگوی `page.goto: Timeout` و پرش به `/login`. **اشتباه
ترتیب‌دهی از من بود:** R9 را در پس‌زمینه گذاشتم و بعد وسطش مهاجرت زدم و PostgREST را
restart کردم. قاعدهٔ ۷ سند restart را واجب می‌کند؛ اجرای هم‌زمانِ مجموعهٔ رگرسیون با آن،
انتخاب من بود و غلط بود.

### اجرای دوم — باطل، و این‌بار مال من نیست

```
شروع ≈ 15:33، مدت 1.2h
15:38:48  تصویر وب دوباره ساخته شد        ← من build نزدم
15:39:41  afrakala-lan-web restart، APP_GIT_SHA: e66e3759 → a19fd811
15:45:09  afrakala-lan-caddy restart
15:53:13  afrakala-lan-auth restart       ← GoTrue
نتیجه: 395 passed / 179 failed
```

### چرا ۱۷۹، و چرا حتی spec‌های خودمان افتادند

۳۱ شکست از آنِ spec‌های M6/M4 بود — همان‌هایی که ساعت ۱۵:۰۵ **۵۹ از ۵۹** سبز بودند.
سنجیده شد، نه حدس:

```
[admin] url=/accounting/bank-accounts denied=true rolesError=false checking=false
[admin] auth/roles responses: (none)
[admin] MAIN: … «بدون نقش» … «دسترسی ندارید. این بخش فقط برای مدیر کل، مدیر، حسابدار است.»
[sales] همان، مو‌به‌مو
```

**«بدون نقش»، و صفر درخواست به `/auth/v1/token` یا `/rest/v1/user_roles`.** نشست‌های
ذخیره‌شدهٔ `e2e/auth/*.storage.json` مرده‌اند — اپ حتی تلاش هم نمی‌کند. هر چهار فایل
access token منقضی دارند (از ۲۰۲۶-۰۸-۰۸) و تا امروز با refresh token کار می‌کردند؛ پس از
restart شدن GoTrue در ۱۵:۵۳:۱۳ دیگر نمی‌کنند.

با نشست مرده، `roles` تهی است، پس **همه** رد می‌شوند — و «admin رد شد» دقیقاً همان چیزی
است که آزمون M6 به‌درستی قرمز می‌کند. یعنی آن ۲۸ شکست، **درستیِ** دروازهٔ M6 را نشان
می‌دهد نه شکستنش: با نقش نامعلوم، هیچ‌کس داخل نمی‌رود.

### آنچه اثبات می‌کند M4 نمی‌تواند علتش باشد

```
git diff --stat e66e3759..a19fd811 -- src/     → خالی
```

کد frontend مستقر با آنچه ۵۹/۵۹ داد **صفر تفاوت** دارد. و این شاخه صفر فایل `src/` دارد.
تغییر M4 هشت predicate در پایگاه‌داده است و نمی‌تواند نشست احراز هویت را بکشد.

### تایم‌لاین کامل، برای اینکه بعداً قابل بازخوانی باشد

```
~15:05  m6-route-guard 59/59، m6-r1 3/3، R1 سبز        ← نشست‌ها زنده، پیش از بازسازی
 15:38  تصویر وب ساخته شد        (مالک)
 15:39  web restart               (مالک)
 15:45  caddy restart             (مالک)
 15:53  auth restart              (مالک)
~16:45  هر نشست: «بدون نقش»، صفر درخواست احراز هویت
```

### وضعیت ثبت‌شده

**R9 = BLOCKED.** نه سبز، نه قرمز — **اندازه‌گیری‌نشده.** قاعدهٔ ۱۱ سند: سه تلاش ناموفق روی
یک قلم → BLOCKED، مانع ثبت شود، ادامه بده.

**آنچه حلش می‌کند:** بازتولید نشست‌های `e2e/auth/*.storage.json`. این کار وارد کردن رمز
است و **من انجامش نمی‌دهم** — spec خود مخزن برایش هست
(`e2e/auth/save-role-sessions.spec.ts`) و کار مالک است. پس از آن، R9 روی محیطی که وسطش
build نمی‌خورد اجرا شود.

**آنچه در نبود R9 هنوز اثبات‌شده است:** این مأموریت پایگاه‌داده است. معیار پذیرشش
سطح-پایگاه‌داده است و کامل سبز است — A1 تا A7، و R2 تا R7 و R10 و R11. R1 و R8 در ~۱۵:۰۵
روی build پیش از بازسازی سبز بودند (۳ از ۳ و ۵۹ از ۵۹) و پس از مرگ نشست‌ها قابل تکرار
نیستند.

### APP_GIT_SHA

سند مأموریت گفته بود روی `e66e3759` بماند و **نماند**: مالک در ۱۵:۳۸ تصویر را دوباره ساخت
و حالا `a19fd811` است — یکی از commitهای همین شاخه، چون درخت کار روی
`feature/m4-viewer-guard` بود. **من هیچ build نزدم.** و چون
`git diff e66e3759..a19fd811 -- src/` خالی است، کد frontendِ در حال اجرا عملاً همان است.

## گام بعدی

ارسال. مأموریت بعدی به ترتیب M5 است و این سند مجازش نمی‌کند.
