...
دور دوم بازبینی، پس از این چهار اصلاح، **باز هم CHANGE REQUIRED** داد و چهار
یافتهٔ تازه آورد. هر چهار پذیرفته و بسته شد:

| # | شدت | یافته | وضعیت |
|---|---|---|---|
| ۵ | **MAJOR** | ادعای «محتوای پاسخ یکسان است» غلط بود — من یک خوراک قیمت عمومی را روشن کرده بودم | بسته — `PUBLISH_PUBLIC_PRICES=false` + **OG-29** |
| ۶ | **MAJOR** | دروازهٔ ۳۷۱ با `security_invoker = on` (به‌جای `true`) دور می‌خورد | بسته — مهاجرت ۳۷۲ |
| ۷ | MINOR | ۳۷۱ نسبت به گرنت سطح‌ستون کور بود | بسته — جاروی `has_column_privilege` در ۳۷۲ |
| ۸ | MINOR | سند اعتبار بیش از حد به بازبین نسبت داده بود | بسته — پایین |

### اصلاح MINOR 8 — آنچه بازبین واقعاً سنجید

نوشته بودم «۲۹ تابع `SECURITY DEFINER` مرتبط همه داخلاً `42501` می‌دهند». این
اغراق بود. بازبین **۹ تا از ۲۹** را آزمود، و از همان نُه‌تا **دو تا** ۲۰۰ با
آرایهٔ خالی دادند نه `42501` (`list_trusted_credit_customers` و
`polymorphic_ref_orphan_report`). درست این است: **۹ از ۲۹ نمونه‌برداری شد، شامل
موارد مالی**؛ ۲۰ تای دیگر آزموده نشد.

### آنچه بازبین در هر دو دور مستقل بازتولید و تأیید کرد

بسته‌شدن نشت روی هر هشت view (۴۰۱/`42501`، با کلید زنده و اثبات‌شده)؛ جدول شمار
ردیف نقش‌ها، هر خانه؛ درستی انتخاب دقیقاً همان دو view؛ نبود view دیگری ساخته‌شده
روی این هشت؛ بسته‌بودن جاسازی FK در PostgREST؛ مسیردهی‌نشدن GraphQL؛ دقت فایل
بازگشت تا سطح `relacl`؛ کامل‌بودن جدول اصلاح‌شدهٔ مصرف‌کنندگان؛ درستی بند ۰.۸ب؛
`npx tsc --noEmit` = **۷۰** (این‌بار خودش اجرا کرد)؛ و اینکه سرویس service-role
سطح عمومی endpoint را گشاد نمی‌کند — ۱۹۹ محصول endpoint **دقیقاً** همان است که
RLS خودِ anon آزاد می‌کند.

و OG-25 را زنده تأیید کرد: یک view تازه‌ساخت بلافاصله `anon=arwdDxt` می‌گیرد و
anon از راهش `current_balance = 10289000000.00` را خواند.
...
`supabase_migrations.schema_migrations` تا `20260811180000` بیشتر ندارد، پس
۳۶۸ تا ۳۷۲ در دفتر آن جدول نیستند. این با روال شناخته‌شدهٔ اعمال دستی مهاجرت روی
سرور تست می‌خواند؛ اثر ۳۷۰ تا ۳۷۲ مستقیماً در کاتالوگ قابل راستی‌آزمایی است.

### یک ارجاع خارج از دامنهٔ جدول مصرف‌کنندگان

`e2e/security/viewer-restrictions.spec.ts` هر شش view باطل‌شده را در
`RESTRICTED_VIEWS` فهرست می‌کند و ادعا می‌کند `viewer` صفر ردیف با
`status < 400` می‌گیرد. **۳۷۰ آن را نمی‌شکند**، چون آن ادعاها روی JWTهای
`authenticated` اجرا می‌شوند و `authenticated` روی هر شش `SELECT` را نگه داشت.
...
این قید در خود فایل به‌صورت کامنت ثبت شد تا ویرایش بعدی ناخواسته کوئری
`products` را هم روی service-role نبرد.

> **اصلاح — بازبینی دوم ثابت کرد ادعای «محتوای پاسخ دقیقاً همان پیش از ۳۷۰
> است» نادرست بود.**
>
> `!inner` در PostgREST با RLS نقشِ فراخوان فیلتر می‌شود، و `anon` **هیچ ردیفی**
> از `sale_price_types` نمی‌بیند (`sale_price_types_auth_read` فقط
> `{authenticated}` است؛ `sale_price_types_read` نقش admin/manager/accountant
> می‌خواهد). سنجیده: مالک ۳ نوع‌قیمت می‌بیند، anon **۰**.
>
> یعنی این endpoint از ۱۴۰۵/۰۵/۱۹ (کامیت `eef3a4a1`) برای **همهٔ** محصول‌ها
> `price: 0` برمی‌گرداند. آن کامیت فیلتر را افزود تا «تماس بیرونی قیمت چکی یا
> همکاری نگیرد»، ولی در عمل کل خوراک را صفر کرد و کسی متوجه نشد.
>
> پس انتقال به service-role فقط ۵۰۰ را رفع نمی‌کرد؛ آن صفرشدن اتفاقی را هم
> برمی‌داشت و **۱۹۳ قیمت نقدی واقعی** را روی endpointی با
> `Access-Control-Allow-Origin: *` منتشر می‌کرد.
>
> شاید همان چیزی باشد که این endpoint از اول برایش ساخته شده. ولی یک افشای
> **بیرونی تازه** است که دیروز وجود نداشت، پس از خزیده‌شدن برگشت‌ناپذیر است، و
> یک تصمیم کسب‌وکاری است — نه چیزی که یک اصلاح امنیتی به‌عنوان عارضهٔ جانبی
> روشنش کند.
>
> **پس روشنش نکردم.** پرچم `PUBLISH_PUBLIC_PRICES = false` در همان فایل، رفتار
> قابل‌مشاهدهٔ endpoint را دقیقاً همان پیش از ۳۷۰ نگه می‌دارد: `HTTP 200`،
> همان ۷ کلید، ۱۹۹ محصول، قیمت‌ها صفر. تصمیم انتشار **OG-29** شد و با یک خط
> قابل روشن‌کردن است.
...
### OG-29 — آیا `/api/public/products` اجازهٔ انتشار قیمت واقعی دارد؟

این Owner-Gate را دور دوم بازبینی تولید کرد، نه سنجش خودم.

وضعیت امروز، سنجیده نه فرض‌شده: این endpoint از ۱۴۰۵/۰۵/۱۹ برای هر ۱۹۹ محصول
`price: 0` می‌دهد، چون فیلتر `sale_price_types!inner(code)` که برای جلوگیری از
افشای قیمت چکی/همکاری افزوده شده بود، با RLSِ anon صفر ردیف می‌گیرد
(anon هیچ ردیفی از `sale_price_types` نمی‌بیند).

اگر `PUBLISH_PUBLIC_PRICES` روشن شود، این endpoint **۱۹۳ قیمت نقدی واقعی** از
۱۹۹ محصول فعال و موجود را با `Access-Control-Allow-Origin: *` منتشر می‌کند.
نام و SKU همین محصول‌ها از قبل به‌عمد عمومی است (`products_public_read`)، ولی
قیمت نیست.

آن را روشن نکردم. دلیل: افشای بیرونی تازه، پس از خزیده‌شدن برگشت‌ناپذیر، و تصمیم
کسب‌وکاری. یک خط در `src/routes/api/public/products.ts` است.

**پرسش از مالک:** آیا قیمت نقدی محصول‌های فعال و موجود عمومی منتشر شود؟
