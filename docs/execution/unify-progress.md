...
| build مستقر | `8766bda7` — هر سه سیگنال تأیید شد |
...
| build نهایی | `8766bda7`، سه سیگنال مطابق |
...
> **نکته دربارهٔ drift:** اگر HEAD یکی-دو commit جلوتر از build مستقر باشد و آن commitها
> فقط مستندات یا SQL باشند، نیاز به rebuild نیست — کد اپ عوض نشده. با
> `git diff --name-only <deployed>..HEAD -- src/ server/` بررسی کنید؛ اگر خالی بود، مستقر درست است.
