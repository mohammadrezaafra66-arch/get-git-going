...
Current phase:        4 COMPLETE — awaiting independent Gate A review; phase 5 not started
...
Migrations applied:   26 (336-361)
Open Owner-Gates:     OG-8, OG-11, OG-12, OG-15, OG-17, OG-21
                      (OG-10, OG-13, OG-16, OG-18, OG-19, OG-20 CLOSED; OG-14 ANSWERED & SCHEDULED).
BLOCKING BEFORE PHASE 6: **reverse_document (OG-14) must be built after phase 4 and before phase 6.**
                      It is not built. Every endorsement mistake is permanent until it exists.
...
| 4 Dual documents | **complete** | 2026-08-19 | 2026-08-19 | 7/7 accept PASS; stress PASS; cleanup proved clean | migrations 360-361. New `dual_documents` table (task 4.2 decision: `mutual_settlements` is a one-party netting table, D10 anticipated it). T11's four roles built - two account holders with journal lines, two record-only with none. 7 contradictions recorded; OG-21 raised. `phase-4-PROGRESS.md` |
...
| OG-14 build reverse_document, or an audited escape hatch? | 2026-08-18 | **2026-08-19** | **ANSWERED — option (b): BUILD IT AFTER PHASE 4, BEFORE PHASE 6.** Reasoning: until phase 6 no real user can reach these forms, so a mistaken document is not yet possible in practice. But **phase 6 is when the accountant and the sales staff get the forms**, and the owner's decision on Gate A B1 (option (a): one cheque is consumed once) makes every mistake **permanent** until reversal exists. It must be in place **before a real user can make one**. Not built yet — it needs its own dispatch. Migrations 353 and 357 both refuse a delete rather than allow an orphan and both name reverse_document as the real cure; migration 356's Persian message tells the user an endorsement cannot be corrected until it exists. **Scheduled between phase 4 and phase 6.** |
...
| OG-21 is a صراف who is paid a fee an account holder? | 2026-08-19 | | Raised by phase 4 (contradiction C-c). **T11** (record-only roles carry no Asan code and generate no journal line), **MASTER-CHECKLIST 4.6** (a non-zero intermediary fee produces a THIRD journal line) and **requirement 207** (the صراف's Asan code deliberately not mandatory) **cannot all be true** - a journal line needs an `account_ref_id` the validator accepts, and a line with no Asan code blocks the whole document from the export (Part 3 rule 2). **Phase 4's reading, implemented and open to being overturned:** a صراف with a ZERO fee is metadata (T11, 207); a صراف with a NON-ZERO fee is a party **we are paying**, so under T10 their balance moves and under T3 they need a code - T11's record-only class covers the transferrer and recipient, who receive nothing. The third line is `('external_party', intermediary_id)`, an existing mapping, zero new ones. **Separately and NOT an interpretation: `p_fee_borne_by='us'` is REFUSED** - it needs a debit to an expense of ours and there is **no expense `account_kind`**; using `other`/`clearing` would silently block the document, and inventing a kind is forbidden by T13 c1. Options: (a) confirm the reading and leave 'us' refused; (b) define an expense account_kind and its Asan code; (c) rule that a fee is never borne by us. Blocks nothing - phase 4 is complete either way. |
