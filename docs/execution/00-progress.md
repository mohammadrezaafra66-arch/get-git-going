...
| OG-32 the public sale-list page is already broken for an anonymous visitor | 2026-08-22 | | **OPEN. Recorded, not fixed — the owner's instruction for exactly this case.** `public.sale-lists.$listId` reads `sale_lists`, `sale_list_items`, `sale_price_types`, `products`, `brands`, `categories` and `refresh_sale_list_prices` through the browser client, which is `anon` for a logged-out visitor. Measured 2026-08-22: `anon` has **no SELECT** on `sale_lists` or `sale_list_items` — it holds `DELETE,INSERT,REFERENCES,TRIGGER,TRUNCATE,UPDATE` on both and not `SELECT`, the strangest ACL shape in the schema and most likely half of a REVOKE somebody started. `GET /rest/v1/sale_lists` as anon returns **401**. So the page cannot render for the audience it exists for, and migrations 374/376 deliberately do not grant those two tables, because granting them would be fixing a broken public surface. **A second defect sits behind the first:** even with those grants, `anon` sees **zero of three** rows of `sale_price_types` (`sale_price_types_auth_read` is `{authenticated}`; `sale_price_types_read` requires admin/manager/accountant), so the price-type title would not render either. This is a second OG-29, not part of OG-25. Note also that **no sale list is published** — all 20 rows are `draft` — so the published path could not be baselined and no test data was created to force it. |
