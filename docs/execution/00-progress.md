...
Current phase:        3 COMPLETE — Gate A reviewed (FAIL) and remediated; phase 4 not started
...
Migrations applied:   23 (336-358)
Open Owner-Gates:     OG-8, OG-11, OG-12, OG-14, OG-15, OG-17, OG-18, OG-19
                      (OG-10, OG-13, OG-16, OG-20 CLOSED).
                      OG-14 URGENCY CHANGED - see its row. OG-18 restated with Gate A evidence.
...
| OG-14 build reverse_document, or an audited escape hatch? | 2026-08-18 | | Gate A M5 - MUST close before phase 9 **URGENCY CHANGED 2026-08-19 (phase-3 Gate A remediation, B1).** This gate was scheduled as "must close before phase 9". The owner's decision on B1 - option (a), the endorsement uniqueness rule is now UNCONDITIONAL - means **one cheque is consumed once, and until `reverse_document` exists a mistaken endorsement CANNOT BE CORRECTED**. That cheque stays consumed. The refusal is the safe behaviour and was chosen deliberately over a permanent double credit, but it makes OG-14 **reachable-urgent the moment anyone endorses a cheque**, not merely a phase-9 prerequisite. `reverse_document` was NOT built in that mission - it is out of scope and needs its own dispatch. The owner decides when. Same applies to migrations 353 and 357, both of which refuse a delete rather than allow an orphan and both of which name reverse_document as the real cure. |
...
| OG-18 does a cheque payment reduce the bank balance on the day it is written? | 2026-08-19 | | **RESTATED 2026-08-19 with Gate A's reproduction attached; still OPEN and deliberately NOT chosen by the remediation mission.** `vw_account_balances`'s outflow CTE and `get_account_ledger` both sum `payment_vouchers.amount` filtered on `status='approved'` with **no `document_channel` predicate**. Gate A reproduced it: `BEFORE out_count=0 total_out=0` -> create a 900,000 own-cheque payment -> `AFTER out_count=1 total_out=900000`, while the entry's lines are `supplier_payable 900000/0` and `cheque_payable 0/900000` - **no bank line exists**. The LEDGER is correct; the two cash views are wrong about the same document. Pre-existing (`pay_purchase_with_voucher` can already write a cheque voucher) but reachable through a supported path since phase 3, and **phase 6 renders both views**. Options: (a) exclude cheques from both readers - the direct mirror of migration 350 on the receipt side; (b) relabel the figure as *committed* rather than *available*. They mean different things to an accountant. Gate A graded it MAJOR. |
