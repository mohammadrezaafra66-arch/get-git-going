...
| OG-43 the repo e2e suite is pinned to a database fixture that no longer exists | 2026-08-24 | | **OPEN — and it is why nobody can currently tell a real regression from drift in this repository.** Run on the deployed build `e66e3759`: **535 passed, 41 failed**. **19 of the 21 distinct failing spec files assert directly against row counts** that later missions changed. The assertion that proves it, from `e2e/asan/export-bank-deposits.spec.ts:64`: `expect(Number(dbScalar("select count(*) from payment_receipts"))).toBe(6)` — the table holds **10**. Its own comment reads *"This spec writes nothing, so rule 2.10 is satisfied by construction — asserted rather than assumed, because 'the test did not write anything' is exactly the kind of claim that rots"*, which is correct about the risk and wrong about which claim rotted: the specs did not write, **other missions did**. The extra rows date 2026-07-25 to 2026-08-20 and one is literally named `OG14_CONC_do_not_keep`, left behind by the OG-14 concurrency work on 2026-08-19. A second spec in the same set fails on the *identity* of the eligible receipts, not just the count. **The cost of leaving it:** a suite that is always red teaches everyone to stop reading it, and this programme has now had one mission (M6) report it green off a truncated capture precisely because nobody expected a number to be checked. **What deciding it would take, and why an agent should not decide it:** the three options are not equivalent and only the owner can choose. (a) Re-pin the constants to today's counts — cheap, but it re-arms the same trap the moment the next mission writes a row, and it silently blesses whatever data is currently there including `OG14_CONC_do_not_keep`. (b) Delete the leftover fixture rows so the pinned numbers become true again — that is **deleting business data from a shared database** and is exactly what no agent may do unasked; it also assumes the four rows are junk, which nobody has established. (c) Rewrite the assertions to be relative rather than absolute — the honest fix and the expensive one, touching roughly 20 spec files, and it changes what those specs actually guarantee. **Prerequisite for all three:** establish which of the four extra `payment_receipts` rows are real business records and which are test residue. Nothing here is fixed, and the specs are left exactly as they are. |
