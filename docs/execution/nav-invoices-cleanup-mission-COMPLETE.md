...
⚠️ **این پروژه `test script` ندارد.** هیچ تست واحد یا e2e اجرا **نشد** و ادعای اجرا نمی‌شود.
اعتبارسنجی رفتاری از راه گیت‌های برابری روی دیتابیس زنده انجام شد (هر دو سمت ROLLBACK).
...
۱. ⚠️ **بررسی مرورگری انجام نشده** — بند ۸. مهم‌ترین مانده.
۲. ⚠️ **`build`/`deploy` روی LAN اجرا نشد.** `afrakala-lan-web` هنوز کد قدیمی را سرو می‌کند؛
   `deploy/lan/build.ps1` + `up.ps1` لازم است تا تغییرات فرانت‌اند دیده شوند. مهاجرت‌ها
   **اعمال شده‌اند**، پس دیتابیس جلوتر از فرانت مستقر است — روت‌های حذف‌شده تا استقرار
   ممکن است خطا بدهند.
