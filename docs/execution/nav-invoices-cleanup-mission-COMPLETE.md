...
کردنشان پشت «تست شکننده است» درست نیست. رفع درست: محدودکردن locator به محتوای اصلی
(مثلاً `page.getByRole('main').getByRole('link', …)`) یا `.first()`. **این کار انجام نشده** —
تغییر spec بدون درخواست، خارج از دامنهٔ این مأموریت بود.
...
۱. **۴ قرمزِ ناشی از کار من رفع نشده‌اند** (بند ب). رفع، ویرایش spec است و درخواست نشده بود.
