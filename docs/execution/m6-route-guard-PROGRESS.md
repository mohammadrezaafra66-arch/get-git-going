...
> **این بخش در ۲۰۲۶-۰۸-۲۴ اصلاح شد. متن اولش غلط بود و متن غلط پایین‌تر با خط‌خوردگی
> می‌ماند، چون در `staging` ادغام شده بود و در بدنهٔ PR #340 هم تکرار شده.**

**نتیجهٔ واقعی، روی build مستقرشده (`APP_GIT_SHA = e66e3759 = HEAD`):**

```
535 passed   41 failed   19 skipped   7 did not run   (32.8m)
```

**R9 قرمز است.**

**~~«490 passed / 19 skipped / 7 did not run / 0 failed / exit code 0 (41.2m)» — صفر شکست.~~**
این را غلط گزارش کردم و علتش ابزارِ خودم بود نه پایگاه‌داده و نه محصول: خروجی را با
`tail -18` گرفتم. پلی‌رایت بلوک شکست‌ها را **بالای** خلاصه چاپ می‌کند، پس آن ۱۸ خط فقط
انتهای خلاصه را داشت، grep من دنبال «failed» گشت و پیدا نکرد، و من «صفر شکست» نوشتم.
از ۱۵ نامی که آن اجرا ثبت کرده بود، **۱۲ تا در فهرست شکستِ اجرای دوم‌اند**. یعنی اجرای
اول هم شکست داشت؛ تعداد دقیقش دیگر معلوم نیست چون خروجی کاملش وجود ندارد.

**هیچ‌کدام از ۴۱ شکست، spec مربوط به M6 نیست.**

**پنج خانواده تک‌تک تشخیص داده شد:**

| خانواده | تعداد | علت سنجیده‌شده |
|---|---|---|
| `e2e/asan/*` | ۲۰ | ادعای گره‌خورده به fixture: `select count(*) from payment_receipts` باید ۶ باشد، **۱۰** است. ردیف‌های اضافه بین ۲۰۲۶-۰۷-۲۵ و ۲۰۲۶-۰۸-۲۰ ساخته شده‌اند و یکی‌شان `OG14_CONC_do_not_keep` نام دارد از ۲۰۲۶-۰۸-۱۹. این ادعا مستقیم از پایگاه‌داده می‌خواند و به build وب کاری ندارد |
| `requirements/218` | ۱ | روی `PaymentReceiptForm` ادعا می‌کند که phase 6 در `e7dc789e` بازنشسته‌اش کرد. صفحه کامل رندر شده بود («ثبت سند حسابداری»، «۱ نوع سند») و `wizard.spec.ts` که خلافش را ادعا می‌کند سبز است — دو spec در تناقض‌اند و ۲۱۸ کهنه است |
| `persons/person-profile:121` | ۱ | با `test.viewer` وارد می‌شود که `status=rejected` است و روی `/login` می‌ماند — OG-36؛ هرگز به `_app` نمی‌رسد |
| `business-flows/211-216` | ۱ | timeout روی کلیک «افزودن به پیش‌فاکتور» |
| `business-flows/213` | ۱ | دستور راه‌اندازی خودش شکست خورد: `docker exec afrakala-lan-db … psql -f /tmp/…sql` |

**۱۹ از ۲۱ فایل spec شکست‌خورده مستقیم روی پایگاه‌داده ادعا می‌کنند.** سه فایل به مسیرهای
گیت‌دار حسابداری می‌روند و هر سه جداگانه بررسی شدند؛ هیچ‌کدام روی گیت M6 گیر نکرده.

**آنچه سنجیده نشد و ادعا هم نمی‌شود:** ۱۶ شکست باقی‌مانده —
`export-sales`، `export-journal`، `export-receipts-payments`، `clusters`، `warehouse`،
`credit-unchanged`، `requirements/215`، `business-flows/212`، `214`، `215` — تک‌تک تشخیص
داده **نشدند**. «مال M6 نیست» برای آنها **از روی الگو** نتیجه گرفته شده (۱۹ از ۲۱ فایل
مستقیم روی پایگاه‌داده ادعا می‌کنند و هیچ‌کدام به مسیر گیت‌دار نمی‌روند)، نه از روی سنجش.

**خط پایهٔ R9 در فاز ۰ گرفته نشد**، پس «همان شکست‌های شناخته‌شدهٔ خط پایه» قابل بررسی
نیست. این کوتاهی من بود و اصلاحش با اجرای دوباره ممکن نیست — خط پایه باید **پیش از**
تغییر گرفته می‌شد.

وضعیت گسترده‌ترش — یعنی اینکه مجموعهٔ e2e این مخزن خط پایه‌ای گره‌خورده به داده دارد که
دیگر با پایگاه‌داده نمی‌خواند — به‌عنوان **OG-43** ثبت شد.
