-- 299-down.sql — reverse migration 299 (directory filters on search_visible_persons)
-- Restores exact migration 298 RPC signature/definition.
-- No indexes were added in 299.
-- No BEGIN / COMMIT.

DROP FUNCTION IF EXISTS public.search_visible_persons(text, integer, integer, text, text[], text, text[]);
...
COMMENT ON FUNCTION public.search_visible_persons(text, integer, integer, text) IS
  'Phase 2 person directory search (migration 298). Restored by 299-down.';
