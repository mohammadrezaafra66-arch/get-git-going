...
-- =====================================================================
-- Rollback for migration 271 (register profiles.person_id in the merge registry).
--
-- Restores the person_merge definition captured before 271 from
-- pg_get_functiondef: docs/verification/pre-271/person_merge.sql
-- (that snapshot already contains the Phase 3 external-parties guard, so this
-- rollback keeps it -- it only removes the profiles.person_id registry entry.)
--
-- WARNING -- WHAT RUNNING THIS COSTS YOU
-- While profiles.person_id exists (migration 270), removing its registry entry
-- makes EVERY person merge abort with:
--   "ادغام متوقف شد: ستون «profiles.person_id» به جدول اشخاص ارجاع می‌دهد ولی
--    سیاست ادغام برای آن تعریف نشده است."
-- That is person_merge's deliberate safety halt, not a bug -- but it means the
-- merge feature is entirely unusable. Only run this together with 270-down,
-- which removes the column. Running it alone breaks /persons/merge.
--
--   docker cp docs/verification/271-down.sql afrakala-lan-db:/tmp/271-down.sql
--   docker exec -e PGPASSWORD=$pw afrakala-lan-db psql -U supabase_admin --     -d afrakala -v ON_ERROR_STOP=1 --single-transaction -f /tmp/271-down.sql
-- =====================================================================
