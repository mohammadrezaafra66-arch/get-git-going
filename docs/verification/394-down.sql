-- ROLLBACK for migration 394 (OG-63 - the Tehran/UTC purchase-date defect).
--
-- Written BEFORE the forward migration (A5.28) and built from the LIVE body captured with
-- `pg_get_functiondef` on the `afrakala` database on 2026-08-26 - not from any file in git.
-- The definition below is that capture VERBATIM, i.e. it restores
-- `IF p_purchase_date > CURRENT_DATE THEN`, the defective comparison, exactly as it stood.
--
-- **Applying this file RE-OPENS OG-63**: purchases become impossible again between 00:00
-- and 03:30 Tehran. That is what a rollback is for, but it should be a deliberate act and
-- not a tidy-up, so the assertion at the end says so with a WARNING rather than staying
-- quiet.
--
-- The 31 distinct Persian-guarded error paths in the body are part of this capture and are
-- restored with it. Deliver by `docker cp` + `psql -f` only - never a pipe, never `-c` (A5.30).
--
-- How to run:
--   docker cp docs/verification/394-down.sql afrakala-lan-db:/tmp/394-down.sql
--   docker exec -e PGPASSWORD=$pw afrakala-lan-db psql -U supabase_admin -d afrakala \
--     -v ON_ERROR_STOP=1 --single-transaction -f /tmp/394-down.sql
...
DO $$
DECLARE v_src text; n_total int; n_distinct int;
BEGIN
  ...
  IF v_src !~ '>\s*CURRENT_DATE' THEN
    RAISE EXCEPTION '394-down: the restored body does not contain the original CURRENT_DATE comparison - the captured state was NOT restored';
  END IF;
  IF position('public.tehran_today()' in v_src) > 0 THEN
    RAISE EXCEPTION '394-down: the restored body still calls tehran_today() - 394 was not undone';
  END IF;
  ...
  IF n_total <> 36 OR n_distinct <> 31 THEN
    RAISE EXCEPTION '394-down: restored body carries % HINT occurrences over % distinct codes, expected 36 over 31', n_total, n_distinct;
  END IF;
  IF position('تاریخ خرید نمی‌تواند در آینده باشد.' in v_src) = 0 OR v_src LIKE '%?????%' THEN
    RAISE EXCEPTION '394-down: Persian in the restored body is missing or corrupted';
  END IF;

  RAISE WARNING '394-down applied: create_purchase again compares against the UTC CURRENT_DATE. OG-63 IS RE-OPENED - purchases will be refused between 00:00 and 03:30 Tehran. All 36 HINT occurrences over 31 distinct codes restored, Persian intact.';
END $$;
