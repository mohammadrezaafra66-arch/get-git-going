-- Down script for migration 320 (enriched Asan journal export descriptions).
--
-- Restores the pre-320 asan_list_journal_export byte-for-byte from
-- docs/verification/pre-320/asan_list_journal_export.live.sql.
--
-- WHAT REVERTING COSTS YOU:
--   * the شرح column goes back to the bare stored line description;
--   * supplier_payable loses its account-code resolver, so EVERY purchase
--     payment entry written by migration 313 becomes blocked with a generic
--     "no Asan code" message that names nothing useful;
--   * mutual_settlement entries lose their doc_kind, so a pure offset
--     settlement (no bank line) falls into 'unclassified' and silently
--     disappears from every filtered export;
--   * the 'settlement' filter value stops being accepted and any caller
--     passing it gets "نوع سند حسابداری برای خروجی معتبر نیست";
--   * the description_quality column disappears - src/lib/asan/export-journal-rows.ts
--     reads it, so revert the frontend alongside this or the badge silently
--     reads undefined.
--
-- No data is touched in either direction: 320 only ever replaced a read-time
-- function.
--
...
DO $$
BEGIN
  RAISE NOTICE '320 reverted: asan_list_journal_export is back to its pre-320 15-column form.';
END $$;
