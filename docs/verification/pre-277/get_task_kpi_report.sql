CREATE OR REPLACE FUNCTION public.get_task_kpi_report(p_days integer DEFAULT 30)
 RETURNS TABLE(section text, bucket_key text, bucket_label text, task_count integer, open_count integer, pending_count integer, in_progress_count integer, done_count integer, blocked_count integer, canceled_count integer, overdue_count integer, due_soon_count integer, avg_completion_hours numeric, completion_rate numeric, overdue_rate numeric, oldest_open_at timestamp with time zone, newest_task_at timestamp with time zone)
 LANGUAGE plpgsql
