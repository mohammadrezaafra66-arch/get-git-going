-- 364-down.sql — reverse migration 364 (reverse_document + reader patches).
--
-- STATEMENTS ONLY. No BEGIN, no COMMIT, no ROLLBACK — the caller owns the transaction (Gate A M7).
--
-- Restores create_payment (356), vw_account_balances + get_account_ledger (359), and
-- asan_list_bank_deposit_export (350), then drops reverse_document.
-- Run this BEFORE 363-down.sql.
