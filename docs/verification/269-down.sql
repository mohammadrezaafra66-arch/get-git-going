...
-- =====================================================================
-- Rollback for migration 269 (one person = one ACTIVE external party, D8-2).
--
-- Restores the definitions captured before 269 from pg_get_functiondef:
--   docs/verification/pre-269/person_create_inline.sql
--   docs/verification/pre-269/person_merge.sql
-- and drops the partial unique index 269 added.
--
-- WARNING -- WHAT RUNNING THIS COSTS YOU
--
-- 1. It re-opens the D8-2 hole: a person may again hold any number of active
--    external parties. There were 0 duplicates when 269 was applied, so
--    reverting does not corrupt anything immediately -- it just removes the
--    rule that keeps it that way.
--
-- 2. person_create_inline's accounting_party branch goes back to INSERTing
--    unconditionally. It was the only one of the three branches without the
--    "reuse the existing legacy row" check that supplier and customer have, so
--    reverting restores an inconsistency, not a design.
--
-- 3. person_merge loses the external-parties both-sides guard. That guard only
--    matters while the index exists, so dropping both together is coherent --
--    but if you drop ONLY the index by hand and keep 269's person_merge, the
--    guard stays and merges are refused for a constraint that no longer exists.
--    Run this whole file rather than cherry-picking it.
--
--   docker cp docs/verification/269-down.sql afrakala-lan-db:/tmp/269-down.sql
--   docker exec -e PGPASSWORD=$pw afrakala-lan-db psql -U supabase_admin --     -d afrakala -v ON_ERROR_STOP=1 --single-transaction -f /tmp/269-down.sql
-- =====================================================================

DROP INDEX IF EXISTS public.uq_external_parties_person_active;

-- ── restore person_create_inline (accounting_party inserts unconditionally) ──
...
-- ── restore person_merge (no external-parties guard) ──
