...
-- ─────────────────────────────────────────────────────────────────────────────────────────
-- SCRATCH ADDITION for the production run. Dry-run v2 finding, 2026-09-01.
--
-- DO NOT "SIMPLIFY" THESE SIX LINES AWAY. They are not duplicates of the line above.
--
-- WHY. `ALTER DEFAULT PRIVILEGES` without `FOR ROLE` only edits the default-ACL rows belonging to
-- the role RUNNING the statement. This migration runs as supabase_admin, so the line above clears
-- supabase_admin's row and nothing else. Production carries rows from TWO grantors:
--
--     grantor        | schema | objtype   | grants anon
--     postgres       | public | FUNCTIONS | yes   <-- the line above never touches this
--     postgres       | public | SEQUENCES | yes
--     postgres       | public | TABLES    | yes
--     supabase_admin | public | FUNCTIONS | yes   <-- the line above clears this one
--     supabase_admin | public | SEQUENCES | yes
--     supabase_admin | public | TABLES    | yes
--
-- and this migration's OWN check C2 scans pg_default_acl for `anon` across EVERY grantor:
--
--     WHERE d.defaclobjtype = 'f' AND ns.nspname = 'public'
--       AND a.grantee = 'anon'::regrole AND a.privilege_type = 'EXECUTE'
--
-- so without the postgres twin the migration clears half the rows and then fails on its own check:
--     ERROR: 393 C2: the public FUNCTIONS default-acl row still grants EXECUTE to anon
--
-- The TABLES and SEQUENCES revokes are here for a second reason: migrations 378, 379 and 380 run
-- immediately after this one and assert that NO anon default-privilege entry remains on TABLES or
-- SEQUENCES in public either. This migration never revoked those at all.
--
-- None of this is visible on the test database, which has ZERO anon default-acl rows, so the
-- original passes there trivially. It is reachable only against production's real state.
--
-- Measured on the pristine clone inside BEGIN … ROLLBACK: with all six, the count of anon
-- default-acl grants in schema public goes 22 -> 0 and C2 passes. supabase_admin is permitted to
-- alter postgres's default privileges; there is no privilege barrier.
ALTER DEFAULT PRIVILEGES                   IN SCHEMA public REVOKE ALL     ON TABLES    FROM anon;
ALTER DEFAULT PRIVILEGES                   IN SCHEMA public REVOKE ALL     ON SEQUENCES FROM anon;
ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public REVOKE EXECUTE ON FUNCTIONS FROM anon;
ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public REVOKE ALL     ON TABLES    FROM anon;
ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public REVOKE ALL     ON SEQUENCES FROM anon;
-- NOTE: schema `storage` also has postgres-granted anon rows. They are deliberately NOT touched --
-- no check in this set looks at `storage`, and Supabase's storage layer legitimately uses anon.
-- ─────────────────────────────────────────────────────────────────────────────────────────
