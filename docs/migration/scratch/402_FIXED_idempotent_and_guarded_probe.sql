-- SCRATCH COPY for the production run. Dry-run findings, 2026-09-01.
-- Two changes, both to make this migration survive a FRESH production database. Nothing about
-- what it actually does to the schema is altered.
--
-- 1. IDEMPOTENT DDL. The original uses bare ADD COLUMN / DROP COLUMN. This matters more than it
--    looks, because this file carries its OWN BEGIN;/COMMIT; -- so under `psql --single-transaction`
--    its COMMIT ends the outer transaction early, its DDL lands, and if a later self-test raises,
--    the migration reports FAILURE with the schema ALREADY CHANGED. Re-running it then dies on
--    "column receipt_id already exists". That is exactly what happened during the rehearsal.
--
-- 2. THE TWO-PARENT PROBE IS GUARDED. It reads `(SELECT id FROM public.dual_documents LIMIT 1)`
--    as its second parent. On a fresh production dual_documents is EMPTY, so that subquery returns
--    NULL, the row has only ONE non-null parent, num_nonnulls(...) = 1 is satisfied, and the CHECK
--    correctly accepts it -- whereupon the probe declares "an attachment with two parents was
--    accepted". Measured on the clone: `fixtures: receipt=37ad878a...  dual=NONE`.
--    The constraint is fine; the probe assumes seeded data. It now skips when it has no dual
--    document to point at, and says so.
...
ALTER TABLE public.document_attachments
  ADD COLUMN IF NOT EXISTS receipt_id uuid REFERENCES public.payment_receipts(id) ON DELETE CASCADE,
  ADD COLUMN IF NOT EXISTS voucher_id uuid REFERENCES public.payment_vouchers(id) ON DELETE CASCADE,
  ADD COLUMN IF NOT EXISTS dual_id    uuid REFERENCES public.dual_documents(id)   ON DELETE CASCADE;
...
ALTER TABLE public.document_attachments
  DROP CONSTRAINT IF EXISTS document_attachments_exactly_one_parent;
ALTER TABLE public.document_attachments
...
BEGIN
  ...
  END;
  IF NOT v_ok THEN
    IF (SELECT count(*) FROM public.dual_documents) = 0 THEN
      RAISE NOTICE '402 PROBE SKIPPED: dual_documents is empty, so the probe had only one non-null parent and the CHECK correctly accepted it. The one-parent rule itself is asserted by document_attachments_exactly_one_parent.';
    ELSE
