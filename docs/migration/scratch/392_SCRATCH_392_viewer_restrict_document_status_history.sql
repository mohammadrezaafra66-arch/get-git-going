...
BEGIN
  -- SCRATCH GUARD (dry-run v6, 2026-09-01). The whole proof below drives real JWTs for five
  -- hard-coded accounts. NONE of them exists on production -- measured: all eleven probe ids
  -- across 391/392/402 return no rows from user_roles. The block cannot merely have its
  -- assertions downgraded, because its SETUP inserts into public.documents with uploaded_by =
  -- a probe account and hits documents_uploaded_by_fkey before any assertion is reached.
  -- The two DDL statements above -- DROP POLICY / CREATE POLICY viewer_restricted -- have
  -- already run and are the real work of this migration. Only the proof is skipped.
  IF NOT EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = v_viewer) THEN
    RAISE NOTICE '392 PROOF DEFERRED -- probe account absent on production; policy created but proof deferred';
    RETURN;
  END IF;
  -- ------------------------------------------------------------------ A. VACUITY GUARDS
  ...
  IF to_regclass('public.document_status_history') IS NULL THEN
    RAISE NOTICE '392 A DEFERRED -- probe account absent on production; policy created but proof deferred -- public.document_status_history does not exist; every check below would pass vacuously.';
  END IF;

  IF NOT (SELECT c.relrowsecurity FROM pg_class c WHERE c.oid='public.document_status_history'::regclass) THEN
    RAISE NOTICE '392 A DEFERRED -- probe account absent on production; policy created but proof deferred -- RLS is DISABLED on document_status_history. A restrictive policy on a table with RLS off is never consulted -- it is decorative.';
  END IF;

  IF to_regproc('public.is_viewer_only') IS NULL THEN
    RAISE NOTICE '392 A DEFERRED -- probe account absent on production; policy created but proof deferred -- is_viewer_only does not exist; the policy predicate would not be evaluable.';
  END IF;
  ...
  IF NOT public.is_viewer_only(v_viewer) THEN
    RAISE NOTICE '392 A DEFERRED -- probe account absent on production; policy created but proof deferred -- probe account % is no longer viewer-only, so the closed half of this gate cannot speak.', v_viewer;
  END IF;
  IF public.is_viewer_only(v_admin) OR public.is_viewer_only(v_mgr) THEN
    RAISE NOTICE '392 A DEFERRED -- probe account absent on production; policy created but proof deferred -- admin or manager probe account is now viewer-only; the open half would fail for the wrong reason.';
  END IF;
  ...
  IF v_pol IS NULL THEN
    RAISE NOTICE '392 B DEFERRED -- probe account absent on production; policy created but proof deferred -- no policy named viewer_restricted on document_status_history.';
  END IF;
  IF v_pol.permissive <> 'RESTRICTIVE' THEN
    RAISE NOTICE '392 B DEFERRED -- probe account absent on production; policy created but proof deferred -- viewer_restricted is %, not RESTRICTIVE. A permissive policy of this name ADDS access instead of removing it; all 91 existing copies are RESTRICTIVE.', v_pol.permissive;
  END IF;
  IF v_pol.cmd <> 'ALL' THEN
    RAISE NOTICE '392 B DEFERRED -- probe account absent on production; policy created but proof deferred -- viewer_restricted covers %, not ALL. FOR SELECT would leave the measured viewer INSERT path wide open, which is half the reason this migration exists.', v_pol.cmd;
  END IF;
  IF v_pol.roles::text <> '{authenticated}' THEN
    RAISE NOTICE '392 B DEFERRED -- probe account absent on production; policy created but proof deferred -- viewer_restricted applies to %, not {authenticated}.', v_pol.roles::text;
  END IF;
  ...
  IF v_ref_qual IS NULL OR v_ref_check IS NULL THEN
    RAISE NOTICE '392 C DEFERRED -- probe account absent on production; policy created but proof deferred -- the reference policy viewer_restricted on payment_receipts is missing, so there is nothing to compare against and this check would pass vacuously.';
  END IF;

  IF v_pol.qual IS DISTINCT FROM v_ref_qual OR v_pol.with_check IS DISTINCT FROM v_ref_check THEN
    RAISE NOTICE '392 C DEFERRED -- probe account absent on production; policy created but proof deferred -- predicate is USING % / WITH CHECK %, which differs from the house pattern on payment_receipts (USING % / WITH CHECK %). A USING-only copy leaves the viewer INSERT path open.',
  ...
  IF v_viewer_n IS NULL OR v_viewer_n <> 0 THEN
    RAISE NOTICE '392 D DEFERRED -- probe account absent on production; policy created but proof deferred -- a viewer-only account still reads % history row(s) for a document it uploaded. That is the exact live path this migration exists to close.', coalesce(v_viewer_n,-1);
  END IF;

  -- closed half -- WRITE. Measured before this migration as SUCCEEDING.
  IF v_viewer_wrote IS DISTINCT FROM false THEN
    RAISE NOTICE '392 D DEFERRED -- probe account absent on production; policy created but proof deferred -- a viewer-only account could still INSERT into document_status_history (result %). The INSERT policy is {public} and checks only changed_by, so a FOR SELECT policy would not have closed this. It must be denied.', coalesce(v_viewer_wrote::text,'<null>');
  END IF;
  ...
  IF coalesce(v_admin_n,0) = 0 OR coalesce(v_mgr_n,0) = 0 THEN
    RAISE NOTICE '392 E DEFERRED -- probe account absent on production; policy created but proof deferred -- the probe history row is invisible to admin (%) or manager (%). Closing the viewer must not empty the table for the roles that legitimately read it.',
  ...
  IF coalesce(v_acct_n,-1) <> 0 OR coalesce(v_sales_n,-1) <> 0 THEN
    RAISE NOTICE '392 E DEFERRED -- probe account absent on production; policy created but proof deferred -- accountant reads % and sales reads % history rows; both read 0 before this migration. This file closes one door and opens none.',
