-- 349 — create_receipt: a receipt is created and posted in one transaction
--
-- Phase 2, tasks 2.2 – 2.8. Contract: docs/api/rpc-contracts.md §1 (reconciled by task 2.1).
-- Reference implementation read before writing: the live body of post_receipt_accounting
-- (pg_get_functiondef, 2026-08-18) — ground-truth §2.
--
-- WHAT THIS REPLACES (eventually). The current create path is four client-side PostgREST
-- inserts in PaymentReceiptForm.tsx, whose "rollback" is a DELETE against a table with no
-- DELETE policy — so it matches zero rows and the orphan is guaranteed, not merely possible
-- (ground-truth §4.1). A4 makes the whole operation one function, so partial creation is
-- impossible by construction rather than by convention. The old path stays live until task
-- 6.9 (D12); nothing calls this function yet.
--
-- WHAT ALREADY DEPENDS ON WHAT THIS TOUCHES — asked before writing, per the mission §G:
--
--   * This migration creates a NEW function. It replaces nothing and drops nothing, so no
--     existing caller changes behaviour. `grep -rn create_receipt src/ server/` → no hits.
--   * It WRITES to payment_receipts, payment_receipt_links, journal_entries, journal_lines,
--     customer_credit_balance/_ledger (via increase_credit) and audit_logs. Every one of
--     those writes goes through the guards that already exist:
--       - trg_payment_receipts_derive_person supplies customer_person_id (NOT NULL) for us.
--       - trg_payment_receipt_links_enforce_limits caps allocations at the receipt amount
--         and at the proforma's remaining balance. It is NOT re-implemented here (CLAUDE.md
--         rule 14); its Persian messages are already written for the user.
--       - trg_validate_journal_line_ref checks every account_ref_id against the table its
--         account_kind implies (341, widened by 347 for OG-10).
--       - trg_journal_entry_immutable / trg_journal_line_immutable make the entry immutable
--         the moment it is written (343). We only ever INSERT, never UPDATE, so we do not
--         collide with them.
--     Nothing here bypasses a rule that already exists; it routes through them.
--   * increase_credit is reused rather than reimplemented. It gates on
--     admin/manager/accountant — the same boundary as this function — and writes its own
--     customer_credit_ledger row and its own 'credit_payment' audit row. See the note on the
--     audit count below.
--
-- ORDER OF OPERATIONS. The contract lists "mint the tracking number" (4) before "assign the
-- document number" (5), but the minted value is INT-<document_number>, so the number must
-- come first. Implemented in the only order that works; the dependency is stated in the
-- contract's own text ("mint p_tracking_number as INT-<doc_number>").
--
-- AUDIT ROW COUNT. audit-trigger-spec.md §2 says each create writes "exactly one" audit row.
-- This transaction writes two: ours ('receipt_created') and one from increase_credit
-- ('credit_payment'), which has written it since long before this programme. The spec's
-- intent — "created but unaudited" must be impossible — holds. Reusing the existing, correct
-- credit function beats writing a second credit path to make a row count come out at one.
-- Recorded as C9 in docs/execution/phase-2-PROGRESS.md.
--
-- IDEMPOTENCY. UNIQUE (source_type, source_id) on journal_entries guarantees one entry per
-- receipt. It is NOT a request-level idempotency key: each call mints a fresh receipt id, so
-- a retried call creates a second receipt. There is no natural key for "the same receipt
-- submitted twice", and inventing one would silently swallow a genuine second payment of the
-- same amount on the same day.
--
-- Rollback: docs/verification/349-down.sql. Run it BEFORE 348-down.sql.
...
CREATE OR REPLACE FUNCTION public.create_receipt(
  ...
  p_amount                       numeric,
  p_payment_date                 date,
  p_payment_time                 time without time zone,
  p_destination_bank_account_id  uuid    DEFAULT NULL,
...
COMMENT ON FUNCTION public.create_receipt(
  text, uuid, numeric, date, time without time zone, uuid, text, text, text,
  date, text, text, jsonb, uuid[]
) IS
  'Creates a receipt and posts its balanced journal entry in one transaction. '
  'Contract: docs/api/rpc-contracts.md §1. Role gate admin/accountant/manager (OG-13). '
  'Migration 349, phase 2.';

-- Grants (task 2.8). Nothing is executable by PUBLIC or anon; the front end calls it as
-- `authenticated` and the role gate above decides. Revoking from PUBLIC first is necessary
