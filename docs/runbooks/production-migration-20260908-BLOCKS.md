...
> ## ✅ این سند اجرا شد و کامل پیش رفت — ۲۰۲۶-۰۹-۱۲
>
> **فاز ۷ PASSED.** لجر **۶۸۱**، بالاترین نسخه `20260912150000`، `APP_GIT_SHA = d60232f5`،
> و `anon` از **۲۱۴** رابطه به **۲۰** رسید. گزارش کامل بلوک‌به‌بلوک با خروجی واقعی هر قدم در
> **`docs/research/production-migration-run-20260912.md`** است — **آن سند روایت شب است؛ این
> سند نقشه‌ای است که با آن اجرا شد.**
>
> **دو چیز در همین سند حین اجرا عوض شد و هر دو زیر علامت خورده‌اند:**
>
> | کجا | چه شد |
> |---|---|
> | **بلوک ۳۲** | مهاجرت **۴۷۷ رد شد** (abort کرد) و **۵۲۳ + ۵۲۴** جایش اجرا شدند |
> | **بلوک ۶۷٫۵** (تازه) | مهاجرت **۵۲۵** برای بستن پنج ویوِ DEFINER اضافه شد |
>
> **پس فاصلهٔ واقعی ۸۰ بود، نه ۷۷.** برای اجرای دوباره روی هر پایگاه دیگر، این دو تغییر
> بخشی از مسیر درست‌اند، نه استثنا.
...
#### و آنچه واقعاً شد — ۲۰۲۶-۰۹-۱۲

جدول بالا **پیش‌بینیِ پیش از اجرا**ست و عمداً دست‌نخورده مانده تا با نتیجه مقایسه شود.

| | پیش‌بینی | واقعی |
|---|---|---|
| فاصله | ۷۷ | **۸۰** (‎+۵۲۳، ۵۲۴ در بلوک ۳۲ · ‎+۵۲۵ در بلوک ۶۷٫۵) |
| اعمال‌شده | ۷۳ | **۷۵** |
| رد شده، ردیف لجر ثبت شد | ۱ (۴۶۰) | **۲** — ۴۶۰ و **۴۷۷** |
| رد شده، ردیف لجر ثبت نشد | ۳ | **۳** — ۴۴۹، ۴۵۰، ۴۵۲ (دقیقاً طبق پیش‌بینی) |
| ردیف لجر در پایان | ۷۴ افزوده | **۶۸۱ کل**، بالاترین نسخه `20260912150000` |
| `anon` روی چند رابطه | — | **۲۱۴ → ۲۵ → ۲۰** |

**یک شکستِ پیش‌بینی‌نشده در ۷۴ مهاجرت: ۴۷۷.** سه توقفِ پیش‌بینی‌شده (۴۴۹، ۴۵۰، ۴۵۲) دقیقاً
با همان پیام‌های پیش‌بینی‌شده افتادند، و دو پیش‌بینیِ بازنگری‌شدهٔ Stage 0 (۴۷۵ و ۵۰۷) هر دو
درست از آب درآمدند.
...
### 🔴 بلوک ۳۲ — **۴۷۷ در اجرای واقعی abort کرد؛ ۵۲۳ + ۵۲۴ جایش نشستند**

> ## این بلوک در ۲۰۲۶-۰۹-۱۲ شکست و جایگزین شد — قبل از اجرا بخوانید
>
> **۴۷۶ نشست. ۴۷۷ کامل rollback شد** با این خطا:
>
> ```
> ERROR:  relation "zz_retired_dynamic_parameter_weights_backup_142" does not exist
> ```
>
> **چرا:** ۴۷۷ پیمایش کاتالوگ نیست؛ یک فهرست **ثابت** از ۳۹۰ دستور `REVOKE` است که از کاتالوگ
> پایگاه **تست** تولید شده. آنجا ۴۵۰ و ۴۵۲ شش جدول را به `zz_retired_*` rename کرده بودند.
> روی پروداکشن آن دو **رد شدند**، پس شش جدول با نام اصلی‌شان مانده‌اند و آن شش نامِ
> `zz_retired_*` اصلاً وجود ندارند.
>
> **جایگزین، و چرا دو مهاجرت نه یکی:**
> - **۵۲۳** = ۴۷۷ منهای **دقیقاً دوازده خط** (شش جدول، هرکدام دو بار: یک بار در بلوک REVOKEِ
>   نوشتن و یک بار در بلوک SELECT). بقیهٔ REVOKEها **بایت‌به‌بایت** یکی‌اند؛ با diff اثبات شد.
> - **۵۲۴** همان شش جدول را با **نام واقعی‌شان** می‌بندد، **به‌علاوهٔ
>   `payment_receipts_backup_20260722`** — جدولی که ۴۵۰ روی تست **drop** کرده بود، پس ۴۷۷ هرگز
>   خطی برایش نداشت. ۵۲۴ کاتالوگ‌محور است (`to_regclass`، رد شدن از غایب)، پس نمی‌تواند مثل
>   ۴۷۷ بیفتد.
>
> **نتیجهٔ اندازه‌گیری‌شده:** `anon_readable_relations` از **۲۱۴ به ۲۵**.
> سپس ردیف لجر ۴۷۷ به‌عنوان skipped-and-superseded ثبت شد — همان الگوی ۴۶۰.

**دستور درست، همین است:**

```bash
mig_apply 20260906140000 20260906140000_476_close_pre393_anon_execute_grants.sql && \
mig_apply 20260912140000 20260912140000_523_close_anon_table_grants_for_production_shape.sql && \
mig_apply 20260912143000 20260912143000_524_close_anon_on_tables_477_could_not_name.sql && \
docker exec afrakala-lan-db sh -c "PGPASSWORD=\"\$POSTGRES_PASSWORD\" psql -U supabase_admin \
  -d postgres -v ON_ERROR_STOP=1 --single-transaction \
  -c \"INSERT INTO supabase_migrations.schema_migrations (version) VALUES ('20260906150000');\""
```

**باید ببینی:** ۵۲۴ هفت بار `now revoked` و بعد `7 present, 0 absent, 7 revoked`، و
`524 VERIFY: none of the seven grants anon anything`. ردیف آخر `INSERT 0 1` است — ثبتِ ۴۷۷
به‌عنوان جایگزین‌شده.

<details>
<summary>دستور اصلی که شکست خورد — برای تاریخ نگه داشته شده، اجرا نکنید</summary>
...
> **این هشدار درست بود ولی جهتش را اشتباه گفت.** نوشته بود «جدولی که روی پروداکشن هست و روی
> تست نیست، بی‌سروصدا رها می‌شود» — یعنی یک نشتِ خاموش. چیزی که واقعاً افتاد **برعکس** بود:
> شش جدولی که روی **تست** با نام `zz_retired_*` بودند روی پروداکشن وجود نداشتند، و
> مهاجرت **به‌جای سکوت، abort کرد**. ریسک درست شناسایی شده بود؛ جهتش نه.
...
### 🔴 بلوک ۶۷٫۵ — مهاجرت ۵۲۵ · بستن پنج ویوِ DEFINER · **حین اجرا اضافه شد**

> ## این بلوک در نقشهٔ اولیه نبود — بلوک ۶۷ آن را لازم کرد
>
> سرشماری بلوک ۶۷ **پنج ویو** را پیدا کرد که `anon` هنوز می‌خواند و **هر پنج DEFINER** بودند
> (یعنی `security_invoker` ندارند). چنین ویویی با هویت **مالکش** اجرا می‌شود، پس **RLS روی
> جدول‌های پایه برای صداکننده اعمال نمی‌شود** — یک `SELECT` ناشناس ردیف برمی‌گرداند، نه مجموعهٔ
> خالی. همین آن‌ها را از هفت ویوِ INVOKER که `anon` هم می‌خواند جدا می‌کند.
>
> **و سه تا از پنج‌تا فقط خواندنی نبودند:** `publish_recipients_view` و هر دو ویوِ ماندهٔ
> سرمایه `anon=arwdDxt` داشتند — یعنی INSERT و UPDATE و DELETE.
>
> **چرا ۵۲۳/۵۲۴ نگرفتنشان:** ۴۷۷ — که ۵۲۳ از آن مشتق شده — فقط `relkind = 'r'` را هدف
> می‌گیرد. **ویوها هرگز در دامنه‌اش نبودند.** این همان نقطه‌کورِ og103 است.
>
> **منشأ گرنت، اندازه‌گیری‌شده نه حدس:** grantor هر پنج ردیف `supabase_admin` است — امضای
> `pg_default_acl`. یعنی گرنت از هیچ دستور `GRANT`ی در هیچ مهاجرتی نیامده؛ موقع **ساخته شدن**
> شیء خودکار اعمال شده. بلوک ۳ آن پیش‌فرض را بست، ولی پیش‌فرض فقط اشیای **آینده** را می‌گیرد
> و به شیئی که از قبل هست نمی‌رسد. پس باید صریح revoke می‌شد.

**paste کن:**

```bash
mig_apply 20260912150000 20260912150000_525_close_anon_on_definer_views.sql && \
docker exec afrakala-lan-db sh -c 'PGPASSWORD="$POSTGRES_PASSWORD" psql -U supabase_admin -d postgres -c \
"SELECT c.relname,
        COALESCE((SELECT o FROM unnest(c.reloptions) o WHERE o LIKE ''security_invoker%''),''(DEFINER)'') AS sec,
...
**باید ببینی:** پنج NOTICE از نوع `anon held …`، بعد
`525: 5 present, 0 absent, 5 had an anon grant to remove`، بعد
`525 VERIFY: the five DEFINER views grant anon nothing; the seven INVOKER views are unchanged`.

سپس در فهرست پایانی **هیچ ردیفی نباید `(DEFINER)` باشد** — هر سطر باید
`security_invoker=true` نشان دهد.

**نتیجهٔ اندازه‌گیری‌شدهٔ ۲۰۲۶-۰۹-۱۲:** `anon_readable_relations` از **۲۵ به ۲۰**، و
`authenticated` روی هر پنج ویو دست‌نخورده ماند.

**اگر هر ردیفی هنوز `(DEFINER)` بود:** بایست و paste کن.

> **چیزی که عمداً انجام **نشد**:** روی این ویوها `security_invoker = true` گذاشته نشد. آن کار
> RLS را به صداکننده می‌بندد و **چیزی را که کاربرانِ واردشده می‌بینند** عوض می‌کند، نه فقط
> `anon` را. مهاجرت ۳۸۶ همین را به‌عنوان **OG-28** باز گذاشته و این سند بازش نمی‌کند.
